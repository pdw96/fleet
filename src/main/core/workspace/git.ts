 ...
 *
 * 이 모듈의 유일한 직접 파일시스템 변이는 `ok()` 재시도 루프의 **`rmSync(index.lock)`** 이다
 * (편집 에이전트가 남긴 stale 락 회수). 게이트 밖인 근거는 형제 예외와 같다 — 게이트의 범위는
 * «LLM 변이·툴 실행·프로세스 spawn»이고(소비자 = `engine`·`orchestrator`·`mcp/host`·`tools/loop`),
 * 이 회수를 부르는 것은 에이전트가 아니라 **엔진의 git 재시도 정책**이며, 워크스페이스 설정 경로는
 * 부팅·태스크 준비 구간이라 승인자가 붙어 있지 않다.
 *
 * **대상 고정은 성립한다**: 경로는 호출자 입력이 아니라 `git rev-parse --git-path index.lock`
 * (실패 시 `<root>/.git/index.lock`)으로 **git 자신이 지정**하고, 지우는 대상은 그 락 파일 하나뿐이다.
 *
 * ⚠ **그러나 「소유 확인」은 성립하지 않는다 — 이 예외는 AGENTS.md 규칙 미충족이며 #285 로 추적 중이다.**
 * 발동 조건(락 계열 stderr 매칭 · 2회차 이후 · `existsSync`)은 **그 락이 stale 하거나 우리 것임을
 * 증명하지 못한다**(Codex PR#282 2R P1). 사용자의 IDE·터미널에서 도는 `git add` 는 누적 ~450ms 를 쉽게
 * 넘기고, 그 **살아있는 락**을 지우면 경쟁 인덱스 writer 가 허용되어 staged 상태가 유실될 수 있다.
 * git 은 `index.lock` 에 소유 PID 를 기록하지 않으므로 파일만 봐서는 소유를 알 수 없다 — 나이·재시도
 * 횟수 같은 간접 신호는 전부 휴리스틱이다(#251 락 층이 채택한 「연령·신원은 소유 증거가 아니다」와 같다).
 * #285 의 방향 = 재시도 루프에서는 삭제하지 않고, **자식 프로세스 트리를 죽인 직후**처럼 소유를
 * 구성적으로 아는 지점에서만 회수한다. 여기서는 그 사실을 **은폐하지 않고 명시**해 둔다.
 *
