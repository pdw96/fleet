 ...
 *
 * destructive 조작을 막는 것은 게이트가 아니라 **대상 고정 + 조건**이다: 경로는 호출자 입력이 아니라
 * `git rev-parse --git-path index.lock`(실패 시 `<root>/.git/index.lock`)으로 **git 자신이 지정**하고,
 * 지우는 대상은 그 락 파일 하나뿐이며, 락 계열 stderr 매칭 + 2회차 이후 + `existsSync` 확인을 모두
 * 통과했을 때만 실행된다. 실패는 삼키고 다음 회차에서 재확인한다(강제 삭제로 밀어붙이지 않는다).
 *
