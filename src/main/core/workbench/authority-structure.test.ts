...
describe('durable-fs.ts — 실 어댑터 두께 상한', () => {
  ...
  // 이 레포는 「왜 이렇게 했는가」를 주석으로 남기는 것이 규율이므로, 물리행을 조이면 그 규율에
  // **주석을 줄이라는 압력**을 거는 셈이고 그것은 이 가드가 지키려던 것(규칙이 어댑터로 새지 않음)과
  // 무관하다. 그래서 물리행은 **완만한 안전망**으로만 두고(파일이 통째로 다른 성격이 되는 것만 잡는다)
  // 실제 게이트는 아래 코드행이다 — 현재 202행 중 코드는 **97행**이다.
