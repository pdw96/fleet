 ...
 *
 * ⚠ **정직**: 부분 쓰기를 실물로 재현하려 파이프(1MiB ≫ 64KiB 버퍼)에도 태워 봤으나 **블로킹 fd 는
 * 전량을 쓸 때까지 블록**할 뿐 부분 반환을 내지 않았다(Docker 실측). 즉 이 루프가 실행되는 조건을
 * 이 레포의 어떤 실 경로에서도 아직 관측하지 못했다 — 근거는 관측이 아니라 **API 계약**(반환값이
 * 「기록된 바이트 수」로 규정됨)과 **Node 자신의 구현**(`writeFileSync` 가 같은 루프를 돈다)이다.
 * 그래서 방어는 유지하되 「실측으로 확인된 필요」라고 쓰지 않는다.
 */
