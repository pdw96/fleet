 ...
 *  - #47 OpenAI reasoning_effort 패리티: SessionsPanel 의 Thinking effort 셀렉트 게이트가
 *    anthropic|openai 에선 보이고 google 에선 숨는다(provider 매핑과 원자적으로 확장된 게이트).
...
test('#47 Thinking effort 게이트가 anthropic|openai 에서 보이고 google 에서 숨는다', async () => {
  ...
  // google → 닫힘(매핑 미구현 — 게이트만 열면 무성 무시이므로 닫혀 있어야 함)
