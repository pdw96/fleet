...
- [ ] **graceful drain(#216 C3) — 배포 컨텍스트**: 진행 중 런이 있는 상태에서 배포 컨테이너 `docker stop` →
      `docker logs <fleet> | grep '\[fleet\] draining'` 관측 → 진행 런이 상한(`FLEET_DRAIN_TIMEOUT_MS`) 안에서
...
확인하라. 보조로 `docker logs <fleet> 2>&1 | grep -E '복호화 실패|영속되지 않는다'` 가 비어야 한다.
...
- **재암호화 경로가 없다.** 기존 암호문은 그 세션을 **같은 id 로 재등록**할 때만 새 키로 덮인다. 자동
  마이그레이션은 없으므로 로테이션 후 **모든 API 키를 수동 재등록**해야 한다.
...
# 5) 각 API 키를 오케스트레이션 UI 에서 재등록 — 반드시 '같은 provider/session id' 로(그래야 upsert 가 고아
#    암호문을 자동 덮는다. 다른 id 로 등록하면 고아가 남아 수동 프룬이 필요해진다).
# 6) 검증: 라이브 세션 목록이 기대대로 채워졌는지 확인(권위). 4)의 grep 이 이후 재기동에서 비면 완료.
```
