...
⚠ 다만 `v0.1.1` 은 파이프라인 사고로 **자산 0개**로 발행돼 사용자에게 배달된 첫 출하는 `v0.1.2` 다
(원인·재발 방지는 아래 체크리스트 1·5항).
...
4. **채널 격리 규칙** — **프리릴리스 태그(`v1.2.3-<식별자>`)는 반드시 stable 이 아닌 피드로 나가야
...
6. **산출물 확인** — 릴리스 자산에 인스톨러와 **그 태그가 나간 채널의** 업데이트 메타데이터가 모두
   있는지. 부류 존재는 5단계 게이트가 이미 막지만, **채널이 맞는지**는 사람이 본다.
   파일명은 채널을 따른다(4단계의 라우팅과 짝) — stable 태그 → `latest.yml`/`latest-linux.yml`,
   ...
   무업데이트**라 무신호다.
7. **실사용 확인** — 최소 1개 OS 에서 다운로드→설치(경고 우회 절차대로)→기동→업데이트 확인.
