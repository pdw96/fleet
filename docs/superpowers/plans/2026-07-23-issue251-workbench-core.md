...
6. **PR 상한 = 코드 순증 1,900행**(기준선 평균 1,447·최대 1,918). 초과 시 **사전 선언한 분할점**을 적용한다.
   **측정 시점 = 머지 직전 HEAD**(`git diff --numstat origin/master...HEAD` 의 `src/`+`scripts/`+`deploy/`+`e2e/`
   ...
   2.20~4.75」**(정정 51 원안 2.66~3.28 → 정정 92 가 4 PR → 정정 139 가 6 PR → **정정 181 이 PR3b 로 하한 갱신**)
   — 「§3 행 개수 × 행당 비용」 모델은 세 번 연속 빗나갔으므로 쓰지 않는다. ⚠ **상한은 이미 세 번
   초과됐다**(PR1b 2,432 · PR1c 1,952 · **PR2b 3,627 = 1.9배**) — 「상한 내였다」를 후속 PR 추정 근거로
   상속하지 않는다. 착수 전 트리거는 순증이 아니라 **프로덕션 물리행**(리뷰 라운드가 거의 늘리지 않는
   축)으로 환산해 선언한다 = 1,900 ÷ **4.75**(최악 배수) ≈ **400**. ⚠ **기록 규율**(정정 139): PR 본문의
   최종 분량은 **머지 직후 `origin/master` 기준 재측정값**이다 — 푸시 시점 수치를 최종으로 적는 실수가
   PR2b·PR2c·PR3a **3연속** 재발했다(각각 3,297→3,627 · 1,464→1,701 · 1,143→1,384).
...
| **PR3a** | `GitRepo` 완성(열거·프로브 포함 11메서드) · 능력 프로브 · R-5 삭제 없는 재시도 | T11·T12 | **실측 1,384**(프로덕션 348 · 배수 3.98) | 없음(상한 내 · 73%) |
| **PR3b** | 통합 WAL **저널 코어만** — 레코드·파서·내구 쓰기·tmp 문법/배치·열거 필터·전이 술어 | T13a′(정정 159 축소판) | **리뷰 중 실측 2,342**(프로덕션 919 · 배수 **2.55**) — 최종은 머지 직후 재측정 | **상한 23% 초과**(정정 181 · 은폐하지 않는다) |
| **PR3c** | **revision-앵커 + 복구 판정**(한 축·한 커밋) — 앵커 정렬 키 = 권위 `revision`(정정 190) · `resultRef` 문법 소유(정정 192) · 전이 불변식 계층 · 판정 함수 | T13b′(+정정 70·85 이월분 · §3-T34·T18b·I11·T33 **문면 개정 포함**) | ~1,400(프로덕션 목표 ≤400) | 착수 전 라운드가 재도출 완료 → **정정 195 가 크레덴셜 축을 PR3c′ 로 분리** |
| **PR3c′** | 구역 capability 민팅 · 크레덴셜 관측 통합 상태 결속 · 저널 `append` 인가 복원 | 정정 187 이관분(§3-T69 재문안) | ~900(프로덕션 목표 ≤300) | 없음 · **PR3c 위에 스택**(운영감사 ③ 시험 대상) |
| **PR3d** | 크래시 tmp 수확기(**2표면** · per-bench) · 스테일 ref 락 관측 | T13c(+정정 78ⓒ·86ⓑ 이월분 · PR3a 실증 라운드 2 이월) | ~1,300(프로덕션 목표 ≤400) | 없음 |

> **순서 = PR3b → PR3c · PR3b → PR3d**(병렬 불가). **PR3b 의 정체성은 「새 모듈만 · 착지 계약 무변경」이며
> 이번엔 참이다**(정정 161) — 2차 라운드의 「최소 개정」 재기술은 복구 판정이 PR3b 에 있을 때의 이야기였고,
> 정정 159 가 그것을 PR3c 로 옮기면서 **축 분할이 복원**됐다. ⚠ **PR3c 는 미결 결정을 안고 있다**(앵커
> 정렬 키 · 회수 어휘 · 활성 집합 이중 정의) — **자기 착수 전 라운드에서 닫는다.** PR3b 는 그 결정들에
> **의존하지 않는다**(그것이 이번 축소의 목적이다).
...
**총 13 PR**(PR2 3분할 + **PR3 5분할** — PR3c 가 정정 195 로 PR3c/PR3c′ 로 갈렸다 · 스펙 §7 "5~6" 대비 +7~8). **은폐하지 않는다** — 스펙 §5 의 1,900행 상한이 §7 의 PR 수 추정보다
...
> 계산한 총량이 상한의 1.3~2배였다. 현재 집계는 위 표(**13 PR** — PR2 3분할 + PR3 5분할)가 권위다
> (CodeRabbit PR#269: 이 줄이 「10 PR」로 남아 있어 같은 절 안에서 105행과 충돌했다 — **같은 드리프트를
> 정정 195 의 재분할에서 반복하지 않도록 두 줄을 함께 갱신했다**).
...
#### PR3b 착수 전 실측 정정 (2026-07-29 · 7렌즈 감사 + 실 git·ULID·fs 3면 실측)

7렌즈 find → 렌즈별 독립 refuter(**후보 66 · CONFIRMED 41 · PARTIAL 24 · REFUTED 1**), 그리고 **하중
프리미티브는 메인 루프가 직접 실측**했다(재현 명령·출력은 PR 본문 인용). ⚠ **refuter 판정 1건을 메인
루프가 뒤집었다**(정정 148) — 「메인 루프 재검증」 규율이 이번에도 값을 했다.

착수 기준선(2026-07-29 · `f48b6e6`): verify 7게이트 **GREEN** · vitest **2590 pass / 57 skip**(108 파일) ·
커버리지 **S93.64 B87.47 F94.65 L94.99**.

| # | 계획/스펙 원문 | 실측 → 판정 | 조치(착수 전 확정) |
|---|---|---|---|
| 132 | 정정 123 「ref-앵커 슬롯 카디널리티 모순 — **PR3b 착수 전 설계 결정**」 | **모순은 실재한다**(실 git 2.54 재현): bench B 에 T1·T2·T3 결과 ref 를 발행하고 「T1 포기 · T2 stale-attempt · T3 완결」을 구성하면 2슬롯에 귀속되지 않는 ref 2건이 남고, `gc --prune=now` + `pack-refs --all` 을 **넘겨 영구 잔존**한다(열거 3건 유지). 즉 §3-T29(형제 ref 공존 = 정상)와 §3-T34(귀속 불가 ref = reconciliation)는 **동시에 GREEN 이 될 수 없다** | **유력안 채택 + 3개 보강.** 권위 레코드에 `highestIntegrationTxnId` 신설, 판정식을 「**사전순으로 그보다 큰 txnId 의 ref 존재**」로 교체. 보강 ⓐ**갱신 시점 = `prepared`**(ref 가 존재할 수 있게 되기 **전**) — `published` 시점 갱신은 **정상 크래시를 롤백으로 오분류**해 복구표(`promote-published`)와 앵커 분기가 정반대 판정을 낸다 ⓑ갱신 연산은 **`max` 이고 `=` 대입이 아니다**(정정 146) ⓒ**전이 불변식**(신·구 레코드 비교)이 필요한데 현행 `checkInvariants(r)` 는 **단일 레코드 전용**이다(authority.ts:896) — 그 신설이 착지 계약 개정이므로 정정 140 이 앵커를 **자기 revert 단위**로 분리한다 |
| 133 | authority.ts:107-108 「git ref 는 권위 파일과 **독립 매체라 동시 롤백이 불가능하다**」 · 스펙 §W-7:775 동일 주장 | **검증되지 않은 안전 주장이 이미 코드 주석으로 착지해 있다.** ⓐ`refs/` 와 `<common gitdir>/fleet/` 는 **정의상 같은 볼륨**(실측: 두 경로 모두 `git rev-parse --git-common-dir` 아래) ⓑgit 은 기본 설정에서 **ref 쓰기를 fsync 하지 않는다**(`core.fsync` 기본 성분 집합에 `reference` 미포함 — 문서상 기본값이며 이 라운드에서 크래시 실측으로 확인한 것은 아니다 · 정직 표기). 따라서 머신 크래시는 **두 매체를 함께 되돌릴 수 있다** | 주장을 **정확한 강도로 재기술**: 「독립 매체 = 서로 다른 파일·서로 다른 쓰기 경로이므로 **한쪽만 되돌아가는 경우를 탐지**한다」이고 「동시 롤백 불가」는 **거짓**이다. 앵커는 **확률적 탐지기**이며 동시 롤백 시 **fail-open** 임을 authority.ts 주석·PR 본문·§W-7 에 명시(PR#266 교훈 「검증 안 된 안전 주장의 문서화」 재발 차단). 이 정정은 **PR3b 첫 커밋에 포함**한다 — 거짓 주석을 이월하지 않는다 |
| 134 | §W-7 복구표:756-766 의 `stage` 열 · 저널 레코드 `stage`(spec:745) ↔ 권위 레코드 `currentIntegrationStage`(spec:368 = authority.ts:117) | **같은 `stage` 가 두 매체에 이중 저장**되는데 순서·권위·불일치 처리가 **포기 경로 1줄**(spec:784)을 빼면 규정 0건이다. 두 매체는 원자적으로 함께 쓸 수 없으므로 불일치는 **구조적으로 발생**하는데 복구표에 그 행이 없다 → 「저널 우선 / 권위 우선 / 저널만」 세 구현이 전부 T32·T63 을 GREEN 으로 통과한다(§3-T63 이 닫으려던 「추론 0」이 저널↔권위 축에서 그대로 뚫린다) | **WAL 규율로 일반화**: 저널이 **선기록**, 권위 CAS 가 **커밋**이다(순서 = 저널 → 권위 CAS). 근거는 스펙이 유일하게 규정한 순서인 포기 경로가 **정확히 그 형태**라는 것(spec:784). 복구표 `stage` 열은 **저널을 읽는다**. **매체 불일치 행 신설**: 저널 stage 가 권위보다 1단계 앞선 상태 = 「커밋 전 크래시」로 정상 분류, 권위가 앞선 상태 = fail-closed(`journal-behind-authority`). §3-T63 을 「저널↔ref」+「저널↔권위」 **2행**으로 분해 |
| 135 | `abandoned` 저널 엔트리 처분 — spec:784(「권위 CAS 후 **엔트리 제거**」) · spec:765(「`finalized`/`abandoned` 잔존 = **청소 복구**」) · spec:1109(「**삭제하지 않고 감사 보존**하되 활성 집합에서 제외」) | **3중 자기모순**(전부 스펙 내부). 대안 ⓒ(저널을 귀속 원장으로 쓰기)의 성립 여부가 여기에 직접 걸리는데 결정이 없다 | **spec:1109 이 후행 개정이자 목적(감사)이 명시된 유일한 문면이므로 그것이 권위**: `abandoned` 는 **보존 · 활성 집합(={`prepared`,`composed`}) 제외**. spec:784 의 「엔트리 제거」는 **「활성 집합에서 제외」로 읽는다**, spec:765 의 「청소 복구」는 **멱등 종결**(삭제 아님). ⚠ 그 대가로 저널이 **무한 누적**되며 이는 정정 145 와 같은 축이다 |
| 136 | §3-T32 「`prepared`/`composed` × ref 유무 **6조합**」 | **계수 오류 + 구성 불가능의 합**(정정 119·114 동형). 실제 복구표는 **5행**이고, 6번째를 만들려면 `prepared × ref값 일치/불일치`가 필요한데 `prepared` 는 **정의상 `resultOid` 가 없어**(spec:746 · 착지 불변식 ③c authority.ts:923-925) **비교 대상 자체가 부재**다 | T32 를 「복구표 **5행 전수** + `REF_NAMESPACE_CONFLICT` 최우선 + 정정 134 의 매체 불일치 2행」으로 재기술. 「`prepared` 는 ref 값 비교가 성립하지 않는다」를 **그 사실 자체를 단언하는 행**으로 남긴다(계수를 완료 조건으로 쓰지 않는다 — 정정 119 교훈) |
| 137 | T13 이 인용하는 §3-T63·T18b·T33·T29·T20 | **네 행이 PR3b 에서 vacuous 다**(4렌즈 독립 수렴): T63 의 순서를 **만드는 시퀀서**(저널 `composed` → `casUpdateRef`)는 **PR5 T18** 소유 · T18b 의 후반 연언(「되돌아간 revision 위에 새 CAS 가 커밋되지 않음」)은 착지 `CasResult` 에 reconciliation 종별이 없고 store 옵션에 `GitRepo` 가 없어 **생산자 부재** · T33 의 네 소비자(실행·통합·`delete-record`·`abandon`)가 **전부 PR3b 밖** · T20 의 raw 해석분은 스펙 행 문면이 정정 85 이후 **개정되지 않아** 빈 귀속이다. ⚠ 스펙 자신이 T63 을 「T17f 등가물」이라 부르면서 **T17f 에만** PR7 이월 처분을 내렸다 | **정직 표기 목록으로 선언**(정정 124 이행): PR3b 가 담당하는 것은 ⓐ「활성 저널 집합 = {`prepared`,`composed`}」 **순수 술어** ⓑ복구 판정 **순수 함수** ⓒ저널 레코드 계약뿐이고, **행동 단언은 PR5(T18)·PR7(부팅)에 명시 귀속**한다. PR 본문에 「이 PR 에서 vacuous」 절을 **사전 계상**한다 — 머지 직전 적발(PR2a·2b·2c 3연속)의 4회차를 만들지 않는다 |
| 138 | PR3b 셀:88 「크래시 tmp 수확기」 | **세 축이 전부 미확정이다**: ⓐ**대상 디렉터리가 갈린다** — 스펙 §W-2:161 은 `tmp/`, 착지 코드는 `authorityDir` **안**(authority.ts:1039-1040)이라 스펙 문면대로 만든 수확기는 **영원히 0건**이다 ⓑ**§3 계약 테스트 행이 0건**(스펙 전수 grep)이라 §1 전제 1「RED 목록이 비면 착수 금지」에 그대로 걸린다 ⓒ**열거 프리미티브가 `DurableFs` 10개에 부재**(durable-fs.ts:67-81)하고 §W-5 개정도 T13 문안 언급도 0건 | ⓐ대상을 **착지 코드 기준 3표면**으로 확정: 권위 tmp(`<area>/authority/<benchId>.json.<ownerToken>.tmp`) · 저널 tmp · 영역 tmp(`.area.json.<hex>.tmp` — coord-area.ts:314 는 `finally` 로 정리하나 **크래시 창이 남는다**) ⓑ§3 행 **신설**(아래 T13 문안) ⓒ**11번째 프리미티브 신설**. ⚠ BUD-7 이 경고한 「실 어댑터 ≤110 코드행 가드에 여유 13행」은 **재측정에서 반박**됐다 — 물리행 **203/280(여유 77)** 이고 코드행 가드도 위임 1줄이면 통과한다. **삭제 근거는 나이·mtime 휴리스틱이 아니다**(실측): bench 락 키 = `benchId`(locks.ts:130-131)로 **배타**이므로 **리스 보유 중 「같은 benchId · 다른 `ownerToken`」 = 구조적으로 죽은 잔재**다. 영역 tmp 는 `REPO_LOCK_KEY` 아래에서 같은 논법 |
| 139 | §1-6 추정 배수 밴드 **2.69~3.34** · 상한 역산 **569** | **stale 이고, 기록된 착지 분량이 3회 연속 머지 트리보다 작다**(정정 74 가 교정한 실수의 재발). 계획 자신의 정의(`git diff --numstat <sha>^ <sha> -- src scripts deploy e2e` 순증)로 **6 PR 전수 재측정**: PR1b 2,432/646=**3.76** · PR1c 1,952/585=**3.34** · PR2a 1,450/539=**2.69** · PR2b **3,627**/1,003=**3.62** · PR2c **1,701**/358=**4.75** · PR3a **1,384**/348=**3.98**. 기록값 대비: PR2b 3,297→**3,627** · PR2c 1,464→**1,701** · PR3a 1,143→**1,384**. 상한 초과도 「두 번」이 아니라 **세 번**이고 최악은 PR2b(**1.9배**)다 | 밴드를 **2.69~4.75**(평균 3.69)로 갱신. 착수 전 트리거 = 1,900 ÷ **4.75** ≈ **400 프로덕션 물리행**(569 폐기 — 최악 배수로 역산하는 것이 상한 트리거의 정의다). **측정 규율 재확인**: PR 본문 분량은 **머지 직후 `origin/master` 기준으로 재측정해 기록**한다(푸시 시점 수치를 최종으로 적지 않는다 — 3연속 재발) |
| 140 | PR3b 셀 「900~1,500 · 분할점 journal >250 → ref-앵커를 PR3c」 | **분할점이 산술적으로 무의미하다**: 셀 자신의 하한 900 을 갱신 밴드로 역산하면 프로덕션 **189~335** 라 트리거(250)가 **거의 확실히 발동**한다 = 조건부가 아니라 이미 확정된 분할이다. 그리고 구성요소별 재도출(형제 모듈 실측 기준: authority.ts 1,620 · coord-area.ts 360 · locks.ts 466 · durable-fs.ts 202)로 PR3b 프로덕션은 **790~1,170**, PR3a 의 추정 과대 배율(1.5~2.1)로 보정해도 **375~780** → 순증 **1,400~3,700**(중심 ≈2,100)로 **상한을 넘는다** | **3분할 사전 확정**(조건부 아님). 경계 근거는 행수가 아니라 **계약 리플 축이 서로 다르다**는 것 — PR0 이 확립한 「유일한 비가역 ripple 이 자기 revert 단위를 갖는다」의 적용이다: **PR3b = 저널 코어 + 복구 판정**(새 모듈 · 착지 계약 **무변경** · 소비자 0) · **PR3c = ref-앵커**(`BenchAuthorityRecord` 필드 신설 + **전이 불변식이라는 새 계층** + §3-T61 전수표·exact 키 핀 개정 = 비가역) · **PR3d = 크래시 tmp 수확기 + 스테일 ref 락 관측**(`DurableFs` 인터페이스 개정 = 또 다른 계약 리플). 각 PR 목표 프로덕션 **≤400**. 총 PR 수 **10 → 12** — 은폐하지 않는다 |
| 141 | 스펙 §1:377-382 「계획 단계에서 **파일 배치까지 확정**한다 · 정의 없는 타입을 구현이 임의 창작하지 않는다」 | **정정 116 의 동형 재발**(§W-6 분은 PR3a 가 닫았는데 §W-7 분이 그대로): `IntegrationTxnRecord` = 스펙 **1건**뿐(spec:736) · `AbandonReason` = 스펙 3건뿐 · 복구 판정 6어휘(`REF_NAMESPACE_CONFLICT`·`RESULT_REF_UNATTRIBUTED`·`RESULT_REF_MISMATCH`·`no-mutation`·`promote-published`)는 **표 셀에만** 있고 대응 유니온 타입이 레포 전수 **0건**. 게다가 표기 규약이 갈린다 — 그 3개는 SCREAMING_SNAKE 인데 착지한 판별 유니온은 전부 kebab | 착수 전 배치 확정: **`src/main/core/workbench/journal.ts`**(스펙 §5 파일표 spec:1123 이 이미 열거)에 `IntegrationTxnRecord`·`AbandonReason`·**복구 판정 판별 유니온 1개**를 둔다. 표기는 **kebab 통일**(`'ref-namespace-conflict'` 등). ⚠ 저널의 버전 필드는 스펙이 `schema` 인데 권위는 `schemaVersion` 이다 — **`schemaVersion` 으로 통일**하고 정정 86 의 「`schemaVersion` **최우선** 검사 → `incompatible-version` 분류」를 **저널에도 적용**(구 버전이 신 버전 저널을 `invalid` 로 오분류해 지우는 I12 재발 차단) |
| 142 | `IntegrationStage` 5값 | **합법 전이 그래프가 타입·스펙·코드 어디에도 없다.** 권위 CAS 는 신 레코드를 **단독으로만** 검사하므로(authority.ts:896 `checkInvariants(r)`) `published → prepared` 역행이나 `abandoned → composed` 부활이 **어느 층에서도 차단되지 않는다** | 전이 그래프를 **저널 모듈에 순수 술어로 확정**(`prepared→composed→published→finalized` · 임의 단계 → `abandoned` · 그 외 전부 거부)하고 **저널 쓰기가 그것을 강제**한다. 권위 레코드 쪽 전이 강제는 정정 132ⓒ 의 **전이 불변식과 같은 계층**이라 **PR3c 로 귀속**(같은 비가역 축을 두 PR 에 쪼개지 않는다) |
| 143 | T13 「생성 저널은 `prepared` 규칙 미상속 — **3채널 열거**로 {없음·부분·완전} 결정론 판정」 | **3채널 중 스펙이 이름 붙인 것은 2개뿐**이다(spec:821-822 「경로=f(id)·브랜치=f(slug)」) — 세 번째는 미상이고 8조합을 3분류로 사상하는 규칙도 없다. 게다가 그 판정의 **생산자는 PR4 T16**(생성 트랜잭션)이라 **T13·T16 이중 귀속**이며 대응 §3 행은 **0건**이다 | 3채널을 **①저널 엔트리 ②worktree 디렉터리 ③git 브랜치**로 확정(스펙의 2개 + 저널 자신). 다만 **판정 규칙만 PR3b 의 순수 술어로 착지**시키고 **행동 단언은 PR4 T16 에 명시 귀속**(정정 137 과 같은 정직 표기). §3 행 신설 |
| 144 | PR3a 실증 라운드 2(계획:1004-1006) 「스테일 ref 락의 관측·회수는 **PR3b 소관으로 명시 이월**」 | **셀에도 T13 문안에도 반영 0건**(조용한 누락). 게다가 「회수」는 R-5 의 **삭제 금지**와 정면 충돌해 형태 자체가 미정이다 | 형태를 **「관측·보고」로 확정**(삭제 없음 — R-5·§W-7:778 「복구 중 자동 삭제 일절 금지」 준수). 조작화는 git 내부 디렉터리를 뒤지지 않고 **행동으로**: `casUpdateRef` 가 R-5 유계 재시도를 소진하고 락 경합 문면으로 실패하면 `ref-lock-stuck` 으로 **분류·표면화**한다. **PR3d 귀속** |
| 145 | §W-7:786 「결과 ref 를 **삭제하지 않는다**」 · 정정 135 의 저널 보존 | **양쪽 다 무한 누적인데 상한도 성능 서술도 어디에도 없다.** 사실상의 유일한 상한은 러너의 **10MB 출력 한도**이고(PR3a 정정 131 이 실측한 축) 그것에 걸리면 `listRefs` 가 **fail-closed 로 bench 를 영구 고착**시킨다. 추가로 스펙은 `published` bench 의 `delete-record` 를 **명시 허용**(spec:1093 I11)하는데 그 순간 **권위 레코드 없는 고아 ref** 가 생겨 어느 판정 분기에도 속하지 않는다 | **비목표로 명시 선언**하고 수치를 남긴다(ref 1건 ≈ 41B packed → 10MB ≈ 24만 건). 고아 ref(권위 레코드 부재)는 **레포 스코프 「정리 필요」 목록**으로 표면화하되 **자동 삭제하지 않는다**(T16 고아 브랜치 처분과 같은 규율). 압축·회수는 **#251 밖 후속 이슈로 등재** |
| 146 | 스펙 §W-7 정렬 전제 「ULID 는 사전순=시간순」 | **밀리초 경계에서만 참이다**(실측). ⓐ알파벳이 ASCII 오름차순(48–57·65–90)이라 **고정폭 시간성분의 사전순 = 수치순**(경계 12값 + 랜덤 100k 쌍 무반례) ⓑ그러나 **같은 ms 안에서는 단조가 아니다** — 같은 ms 로 20,000건 연속 생성 시 **10,012건(50.1%)이 직전보다 작다**(monotonic counter 부재 · 독립 렌즈도 49.78% 로 수렴) ⓒ`ULID_RE`(ulid.ts:29)는 **생성기 범위 밖 시간 성분을 통과**시킨다(`newUlid` 는 48bit 초과를 throw 하나 검증기는 거부하지 않는다) | ⓐ·ⓑ 로부터: 앵커 갱신은 **반드시 `max`**(정정 132ⓑ)이고 그 falsifier 는 **같은 ms 역전 픽스처**다(`=` 대입 뮤턴트가 RED 가 되는지로 확인). ⓒ는 **fail-closed 방향**(외부가 만든 과대 ref 이름은 영구 reconciliation)이므로 **잔존 위험으로 명시 선언**하고 위협 모델(§W-2-a 「악의적 변조 방어는 비목표」)에 귀속 |
| 147 | 정정 123 의 스펙 라인 인용(`:712-715`·`:726`) | **stale**. PR3a 착지로 밀려 현재 그 줄은 판정식·포기표가 아니라 **통합 계산 코드펜스·`gc --prune` 정직 단서**를 가리킨다. 실제 위치는 판정식 **:772-775** · 포기표 **:786** | 인용 갱신(정정 120 이 「라인 인용은 PR 마다 밀린다」고 경고한 축의 **동형 재발** — 이번엔 계획 자신의 정정이 밀렸다). **후속 규율**: 정정표에 스펙 라인을 적을 때 **절 번호(§W-7)를 1차 키로, 라인을 보조로** 쓴다 |
| 148 | ⚠ **메인 루프가 뒤집은 refuter 판정**(정직 기록) | INV-6 은 「포기 행이 `currentIntegrationTxnId → null` 만 지시하는데 착지 불변식은 3필드 동시 소거를 강제하므로 **포기가 원리적으로 불가능**」이라 P2 CONFIRMED 를 받았다. **거짓이다** — 불변식 ③(authority.ts:906-908)은 「함께 존재하거나 함께 부재」이므로 포기 CAS 가 **4필드를 함께 지우면 그만**이고, 그 형태의 선례가 이미 착지해 있다(`reclaimDraft` authority.ts:587) | 살아남는 정확한 주장으로 **강등**: 「spec:785 포기표가 **과소 명세**다 — 4필드 동시 소거를 적어야 한다」(P3). 불변식은 결함을 **RED 로 잡는 쪽**이지 막는 쪽이 아니다. ⚠ 이 건은 PR#266 교훈(「refuter 의 잘못된 강등이 취약점을 통과시켰다」)의 **반대 방향 사례** — refuter 가 **과대 판정**도 한다 |

**이 라운드가 바꾼 것 요약**: PR3b 는 **착수 불가 상태였다**. ⓐ정정 123 의 설계 결정 미완(T34 착지 금지) ⓑ수확기가 §3 행·프리미티브·대상 디렉터리 3축 미확정 ⓒ네 계약 테스트가 vacuous ⓓ예산이 상한을 넘는데 분할점이 산술적으로 무의미. 정정 132~148 이 넷을 전부 닫았고, 그 대가로 **PR3b 는 3 PR(3b·3c·3d)로 분할**되며 총 PR 수는 10 → 12 가 된다.

#### 2차 라운드 — 결정 4건 적대 반증 (2026-07-29 · Codex 무응답 fallback)

체크포인트(#251 `issuecomment-5117337703`)에 `@codex review` 를 걸었으나 **1시간 25분 무응답**이었다
(축소판 재트리거에 👀 가 3분 만에 붙었다가 45분 뒤 **리액션마저 사라지고 코멘트 0건** — 이 이슈에서
관측된 적 없는 형태다). 이 이슈의 선례(레이트리밋 52분 중 로컬 5렌즈가 P1 12건을 전부 실측 확정)를
따라 **리뷰의 실질을 로컬 적대 반증으로 수행**했다 — 7기가 결정 하나씩을 **깨는 것을 목표로** 붙었다.

**결과: 53 공격 중 33 성공 · 결정 2와 결정 4는 `BROKEN` · 나머지 전부 `WEAKENED`.** 「리뷰 먼저」가
옳았다 — 1,400행을 쓰기 전에 내 결정 중 **둘이 틀렸고 하나는 근거가 틀렸다**는 것이 드러났다.

| # | 1차 결정 | 반증 → 판정 | 2차 확정 |
|---|---|---|---|
| 149 | 정정 132ⓑ 「갱신 연산은 **`max`**」 | **깨졌다.** `max` 는 **비단조를 삼키는 연산**이라 「txnId 사전순 == 권위 세대순」이라는 **미기재 전제**를 만들지 못한다. 시나리오: T_a prepared(highest=T_a) → 포기 → **호스트 시계 역행**(NTP 스텝·VM 재개 — `newUlid` 는 `Date.now()` 를 그대로 쓰고 직전 발행값 대조가 없다 · ulid.ts:66) → T_b(&lt;T_a) prepared 시 `max` 가 highest 를 **T_a 로 유지** → T_b 의 ref 발행 후 권위 단독 롤백 → 열거 {T_b}, T_b &lt; highest → **앵커 침묵**. 게다가 다음 txn 이 워터마크를 올려 **롤백 증거가 영구 소멸**한다. 정정 146ⓑ 의 같은 ms 50.1% 역전은 시계 역행 없이도 같은 결과를 만든다 | **`max` 폐기 → strict 전진 강제.** prepared CAS 가 **`T_new > old.highest`** 를 **전이 불변식**으로 요구하고 위반 시 fail-closed. 시계 역행을 정상 운영 조건으로 흡수하려면 ULID 민팅에 **워터마크 클램프 seam**(`now = max(Date.now(), decodeUlidTime(highest)+1)` — ulid.ts 는 이미 `now` 주입 seam 을 갖는다). 그러면 「txnId 순 == 세대 순」이 **가정이 아니라 집행**이 되고 연산은 `=` 로 족하다 |
| 150 | 정정 132 판정식을 복구표의 **행**으로 둠 | **깨졌다.** ⓐ앵커가 발화하는 유일한 상황(권위 단독 롤백)에서 복구표는 저널 stage 로 `promote-published`(spec:762) 또는 「정상 대기」(spec:764)를 답하는데 **우선순위 규정이 없다** — 표 순서대로 first-match 하는 구현은 **되돌아간 revision 위에 새 CAS 를 커밋**한다(§3-T18b 가 금지한 바로 그 결과). ⓑ입력 **카디널리티가 다르다**(표 = txn 1건 / 앵커 = bench 의 ref N건)는 사실이 시그니처에 없어, PR3c 가 앵커를 얻으려면 **PR3b 순수 함수 시그니처를 개정**해야 한다 → 정정 140 이 3분할 근거로 내건 「PR3b = 착지 계약 무변경」이 파기된다 | 앵커를 표의 행이 아니라 **`REF_NAMESPACE_CONFLICT` 와 같은 계층의 선행 게이트**로 승격하고 그 사실을 표 밖 문면에 못박는다. **PR3b 판정 함수 시그니처에 앵커 입력을 처음부터 포함**한다 — `{record, journalEntries, refs} → verdict` · 앵커 값 미주입이면 **`anchor-unavailable` fail-closed**(옵셔널로 두면 PR3c 이전 구현이 조용히 앵커 없이 판정한다) |
| 151 | 스펙 §W-7:766 「win32 `file-only` **전용** 4번째 분기」 | **깨졌다**(메인 루프 직접 확인). `probeDurability`(durable-fs.ts:184-201)는 **POSIX 에서도** `openDir`/`fsync` 가 던지면 `catch` 로 **`'file-only'` 강등**한다 — dirfd fsync 를 지원하지 않는 마운트(gRPC-FUSE·9p·일부 NFS) 위의 리눅스 컨테이너가 정확히 그 경우다. 문면대로 `platform === 'win32'` 로 게이팅하면 **정작 필요한 표면에서 탐지기가 꺼진다** | 발화 조건을 **`writtenBy.durability === 'file-only'`** 로 확정(등급은 이미 레코드에 있다 · authority.ts:123-127). 「win32 전용」 문면은 스펙 §W-7:766·:357-359 양쪽에서 폐기 |
| 152 | 정정 133 재기술 「독립 매체 = **한쪽만 되돌아가는 경우를 탐지**한다」 | **여전히 과대하다.** 권위 파일은 rename **전에 fsync** 되는데(authority.ts:1429-1430) loose ref 는 git 이 fsync 하지 않으므로 **ref 가 더 약한 매체**다. 「권위는 살아남고 ref 만 유실」 시나리오에서 ⓐ앵커는 ref 의 **부재**를 보지 않아 침묵 ⓑ복구표 `published` 행은 ref 값 열이 `—` 라 부재를 보지 않는다 → **결과 커밋을 가리키던 유일한 ref 가 사라졌는데 bench 가 integration-ready 로 무기한 대기**한다. 즉 앵커는 둘 중 **덜 일어나는 쪽**을 탐지하도록 배치돼 있다 | ⓐ복구표 `published` 행의 ref 열을 열고 **`published × ref 부재` 행 신설**(reconciliation · 자동 재시도 금지 §W-7:778 유지) ⓑ정정 133 문면을 **「권위 쪽 롤백만 탐지 — ref 쪽 롤백과 동시 롤백은 미탐지」** 로 더 좁힌다(현 문면은 대칭 탐지를 시사한다) |
| 153 | §3-T18b·T34 문면을 그대로 둠 | **깨졌다.** 새 판정식에서 T18b(「**이전 세대** 바이트로 되돌린 뒤 재부팅 → 앵커가 reconciliation 을 강제」)는 **구성 불가능**하다 — 한 txn 의 세대열 안에서는 어느 세대로 되돌려도 highest 가 그대로다. 구현자는 RED 를 없애려 판정식을 **구 형태(영구 오탐)로 되돌리거나** 테스트를 조용히 바꾼다. 그리고 T34(spec:1204)는 **구 판정식 문면 그대로**라 T29(형제 ref 공존=정상)와 동시 GREEN 불가라는 모순이 §3 에 잔존한다 | **결정과 같은 커밋에서 §3 문면을 고친다.** T18b = 「**prepared CAS 를 포함해** 되돌린다(= highest &lt; 최신 ref 가 되는 세대)」 + **음성 통제 짝**(「prepared 이후 세대로만 되돌리면 앵커는 발화하지 않고 매체 불일치 행이 답한다」 — 없으면 「무조건 reconciliation」 구현이 통과). T34 = 「highest 보다 사전순으로 큰 txnId 의 ref」로 전면 재문안하고 **T29 와의 동시 GREEN 을 같은 픽스처로 한 행에서 단언** |
| 154 | 정정 134 「저널 선기록 → 권위 CAS」 (결정 2) | **BROKEN.** ⓐ권위 CAS 의 **모든 값-거부**(`invariant-violation`·`revision-mismatch`·`lease-invalid`·`io-failure`·`commit-uncertain`)가 「귀속 불가 **활성** 저널 엔트리」로 변환된다 — 활성 집합 = {`prepared`,`composed`} 는 차단이므로 그 bench 는 통합·삭제·포기가 **전부 거부되는 영구 고착**이 된다. 이것은 정정 132 가 ref 매체에서 닫은 **카디널리티 모순의 저널 매체 복제본**이다. ⓑ매체 불일치 2행의 **정의역이 미확정**이라 비-current 엔트리 N-1 개에서 진리값이 없다 — 「비-current 도 비교」로 읽으면 **정상 포기 이력 1건이 레포를 영구 차단**한다 | **3조 재설계**: ①**abort-on-CAS-failure 를 명시 계약으로** — 각 단계 = (저널 쓰기, 권위 CAS) 쌍이고 **어느 쪽이 실패해도 트랜잭션은 다음 단계로 전진하지 않는다**(이것이 없으면 CAS 실패 후에도 ref 가 발행될 수 있어 앵커 불변식까지 깨진다) ②**귀속 규칙** — 저널 엔트리는 **권위가 참조할 때만 활성**이다. 귀속 없는 `prepared`/`composed` 엔트리 = 커밋 전 크래시 = **`never-applied`** 로 회수 적격(①이 downstream 부작용 0 을 보장한다 · §W-9 생성 저널의 같은 규칙과 정합) ③**매체 비교 정의역 = current txn 1건**. 비-current 는 stage 비교가 아니라 **귀속으로 판정**한다 |
| 155 | 정정 138 수확기 「3표면 · 리스 배타 + `ownerToken` 불일치」 (결정 4) | **BROKEN.** ⓐ영역 tmp 이름은 `.area.json.<randomBytes(8) hex>.tmp`(coord-area.ts:314)라 **benchId 도 `ownerToken` 도 들어 있지 않다** — 판정식이 그 표면에서 **표현 자체가 불가능**하다(내가 그 줄을 직접 읽고도 결정에 반영하지 못했다). ⓑ레포 1패스 수확은 **남의 라이브 tmp 를 지운다** — authority.ts:1131-1139 가 in-process 판으로 이미 P1(Codex PR#267)로 닫은 사슬의 **크로스 락스코프 재개**다 ⓒ표면은 3이 아니라 **4**(인스턴스 tmp 누락) ⓓ저널 tmp 이름 문법을 정하는 것은 **PR3b** 이므로 PR3d 가 PR3b 에 **선행 의존**한다 → 「계약 리플 축이 서로 다르다」는 분할 근거가 이 지점에서 무너진다 | **표면별로 배타원을 따로 적는다**(1열 추가). 권위·저널 tmp = 「**그 benchId 의 리스를 보유한 구간에서 그 benchId 것만**」 — 수확기는 **레포 1패스가 될 수 없다**. **영역·인스턴스 tmp 는 수확 대상에서 제외**한다 — 이름에 소유 증거가 없어 안전한 판정식이 존재하지 않고, 생성이 **레포당 1회 + 경합 시 소수**라 누적이 유계다(비목표로 명시 선언 · 조용한 누락 금지). **저널 tmp 문법(`<txnId>.json.<ownerToken>.tmp`)과 「저널 쓰기는 bench 리스 아래에서만」을 PR3b 계약으로 못박고**, 「PR3b 가 이 문법을 확정하지 않으면 PR3d 는 다시 착수 불가」를 선행조건으로 적는다 |
| 156 | 정정 133 의 **근거** 「`core.fsync` 기본 성분 집합에 `reference` 미포함 — **문서상 기본값**」 | **결론은 옳고 근거가 틀렸다**(git 소스 대조로 확정). git **문서**에는 「기본 성분 집합」 열거가 **없고**, 기본값 문장(`core.adoc:663-666`)은 「`committed,-loose-object` 와 등가」인데 같은 문서의 성분 목록(`:679-682`)은 「`committed` 는 현재 `objects` 와 등가」라고 적혀 있다 — 후자는 **2.36.0 이래 stale** 이고(코드의 `COMMITTED` 는 도입 때부터 `OBJECTS｜REFERENCE`), **문서 두 문장을 합성하면 정반대 결론**이 나온다. 즉 리뷰어가 문서만으로 검증하면 정정 133 은 **형식상 성립하는 반박**을 받는다 | 근거를 **git 소스 4점 + 호출부 2점**으로 교체: `environment.c:62`(`fsync_components = FSYNC_COMPONENTS_DEFAULT`) · `write-or-die.h:33-35`(DEFAULT = pack·pack-metadata·commit-graph — **REFERENCE 부재**) · `write-or-die.c:78-83`(비트 없으면 no-op) · `write-or-die.h:51-52`(플랫폼 오버라이드 훅이 upstream·GfW 어디서도 재정의 안 됨) + `refs/files-backend.c:2029`(2.39.5 `:1793`) · `refs/packed-backend.c:1554`. **버전 축 확인**: 2.36.0~2.54.0 · Debian bookworm 2.39.5 · GfW 2.54.0.windows.1 **전부 동일**(오버라이드 0건). ⚠ **보너스 2건**: ⓐ`core.fsync=all` 을 켜도 **rename 내구성은 얻지 못한다** — loose ref 게시는 `.lock` 쓰기 + `rename()` 인데 그 경로에 **디렉터리 fsync 가 없다**(`lockfile.c` 에 `fsync` **0건** · 게시는 `tempfile.c:348` 의 맨 rename) ⓑ`GIT_TEST_FSYNC=0` 이 git 의 **모든 fsync 를 전역 무력화**한다(`write-or-die.c:60-63`) — 스폰 env 상속 경로가 이 값을 흘리면 설정 무관 no-op. 둘 다 문면에 남겨 **후속 PR 이 잘못된 완화책을 채택하는 것을 사전 봉쇄**한다. 그리고 ⓐ「정의상 같은 볼륨」은 **약화**한다(정션·심링크·reftable 배치에서 갈릴 수 있다 — 다만 갈려도 순서 보장이 강해지지는 않는다 · reftable 도 **같은 게이트**이고 현행 기본 포맷은 `files` 실측) |
| 157 | 정정 136·`REF_NAMESPACE_CONFLICT` **최우선** | **깨졌다**(메인 루프 실 git 재현 — refuter 를 의심했다가 내가 틀렸다). `refs/fleet/integrated/B` 를 bare 로 선점한 뒤 **접두에 슬래시를 붙여** `for-each-ref 'refs/fleet/integrated/B/'` 로 열거하면 **0건**이고(슬래시 없이 질의해야 1건), 그 아래 txn ref 생성은 `'refs/fleet/integrated/B' exists; cannot create …` 로 거부된다. 즉 **D/F 충돌 bench 는 「발행된 ref 없음」과 구분 불가**라 최우선 분기가 **도달 불가**이고, 복구 판정은 `no-mutation`(= 포기·재준비 적격)을 답한다 | 복구 판정 입력에 **두 열거를 모두** 싣는다 — 접두(`…/<benchId>/`)와 **정확 이름**(`…/<benchId>`). 충돌 판정 = 「정확 이름 항목이 존재」. ⚠ PR3a 가 git.ts:169-174 에 남긴 「bare 부모 ref 는 결과에 이름이 정확히 같은 항목으로 나타난다」는 **정확 이름 질의**에 대한 서술이며 **접두 질의에는 성립하지 않는다** — 그 주석에 이 구분을 명시한다 |
| 158 | 정정 140 「PR3b = 새 모듈 · **착지 계약 무변경** · 소비자 0」 | **거짓이 됐다.** 정정 150(판정 함수가 앵커 입력을 처음부터 받는다) · 155ⓓ(저널 tmp 문법이 PR3b 계약) 때문이고, `DurableFs` 11번째 프리미티브를 PR3b 가 흡수하는 갈래에서는 페이크(264행)·`durable-fs.test.ts:13` 「10 프리미티브」 문면까지 함께 개정된다 | 정체성을 **「새 모듈 + 복구 판정 · 착지 계약 **최소** 개정」** 으로 정직 재기술한다(「무변경」 폐기). `DurableFs` 확장은 **PR3d 에 유지**(PR3b 는 열거를 쓰지 않는다). 분할 경계는 유지하되 **의존 순서를 명시**: PR3b → PR3c · PR3b → PR3d(병렬 불가) |

**2차 라운드 교훈**: ⓐ**「리뷰 먼저」 선택이 값을 했다** — 결정 2·4가 코드 0행 시점에 무너졌다. ⓑ**refuter 를 의심한 내 판단이 틀린 경우가 있었다**(정정 157) — 1차 라운드에서 내가 refuter 를 뒤집은 것(정정 148)과 **반대 방향**이라, 「메인 루프 재검증」은 refuter 를 이기려는 절차가 아니라 **양방향 실측 절차**임이 확인됐다. ⓒ**내가 직접 읽은 사실을 결정에 반영하지 못한 사례**(정정 155ⓐ — coord-area.ts:314 를 읽고도 그 이름에 소유 증거가 없다는 함의를 놓쳤다).

#### 3차 라운드 — 재설계안 표적 반증 → **범위 축소 확정** (2026-07-29)

재설계안(정정 149~158)에 4기를 붙였다. **24 공격 성공 · 4대상 중 3이 `BROKEN`.** 지적 수가 줄지
않았다는 것 자체가 신호다 — 이 이슈가 이미 기록한 교훈 「**패치가 안전↔가용성을 번갈아 뚫으면
프리미티브 선택 또는 범위를 의심하라** — 6라운드 내내 장치를 얹었는데 정답은 덜어내는 것이었다」가
그대로 적용되는 국면이다. 그런데 이번 지적들은 서로 다른 방향에서 **하나의 처방으로 수렴**했다.

| # | 3차 지적 | 확정 |
|---|---|---|
| 159 | **`anchor-unavailable` arm 은 오직 「판정 함수가 자기 입력의 생산자보다 먼저 착지한다」는 배치 때문에만 존재한다.** 원인이 배치라면 방어를 추가할 게 아니라 배치를 고쳐야 한다. 그리고 PR3b 는 소비자 0 이라 이 결함이 **PR3b 안에서 관측되지 않고** PR7 부팅에서 「fleet 전체 bench 가 reconciliation」으로 처음 드러난다 | **범위 축소 — 복구 판정을 통째로 PR3c 로 옮긴다.** 앵커 필드·전이 불변식 계층·판정 함수가 **한 커밋에서 함께 태어나면** `unavailable` 상태가 **존재할 수 없다**. 이것은 3라운드 통틀어 **처음으로 장치를 빼는 수정**이고, 정정 140 이 내건 기준(「유일한 비가역 ripple 이 자기 revert 단위를 갖는다」)을 **처음으로 실제로 만족**시킨다 |
| 160 | **PR3b 는 ≤400 에 들어가지 않는다**(형제 실측 기반 재도출: 745~1,125 프로덕션 → 순증 **2,004~5,344**). 넘는 지점은 복구 판정이 아니라 **레코드·파서·직렬화·내구 쓰기 기반부**이고, PR 이름이 된 복구 판정은 441행째부터 시작한다 | 정정 159 의 축소로 해소된다 — 축소된 PR3b(레코드+파서+내구 쓰기+tmp 문법/배치) floor ≈ **300**. ⚠ **「§3 행 개수 × 행당 비용」 모델은 쓰지 않는다**(세 번 빗나갔다 · §1-6) — 근거는 형제 모듈 실측이고 그 수치를 PR 본문에 적는다 |
| 161 | 정정 158 의 「계약 리플 축이 서로 다르다」가 **거짓이 됐다** — PR3c 는 「값을 채우기만」이 아니라 **착지 권위 레코드에 대한 두 번째 수술 + PR3b 가 갓 만든 계약에 대한 첫 수술**이라, 「최소 개정」이 **두 번** 일어난다. 그러면 분할은 축 분할이 아니라 **크기 분할**인데 셀의 분할점은 「없음」이다 | 정정 159 채택으로 **축 분할이 복원**된다: PR3b = **새 모듈만**(착지 계약 무변경 — 이번엔 참이다) · PR3c = **권위 레코드 계약 개정 + 그것을 소비하는 판정**(한 축·한 커밋) · PR3d = `DurableFs` 개정 |
| 162 | **규칙 1(abort-on-CAS-failure)의 강제자가 없다** — 시퀀서는 PR5 T18 소유이고, 부작용 생산자인 `casUpdateRef`(git.ts:181)는 커밋 토큰을 받지 않는다. 「규약은 미래 편집 한 줄로 조용히 깨지므로 구조적 보장이 아니다」(체크포인트 2 교훈)의 정확한 재현 | ⓐPR3b 범위에서 가능한 구조화만 한다: 저널 쓰기 시그니처를 **`appendJournal(entry, prev: AuthorityCommit)`** 로 두어 「직전 단계 CAS 가 `committed` 여야 다음 저널을 쓸 수 있다」를 **타입으로** 강제(선례 = authority.ts:510-519·545-570 의 원장·단일사용 패턴) ⓑ`casUpdateRef` 에 커밋 토큰 인자를 추가하는 것은 **PR5 T18 의 선행조건**으로 계획에 명문화(PR3b 계약 아님) ⓒ「규칙 1 의 **행동** 단언 = 생산자 부재 = 이 PR 에서 vacuous」를 사전 계상 |
| 163 | **저널 단독 롤백이 발행된 ref 를 `never-applied` 로 오답한다** — 정상 published 후 포기 → 저널만 이전 세대로 롤백 → 귀속 소멸 → 회수 적격. 이 사슬에서 **CAS 는 한 번도 실패하지 않았으므로 규칙 1은 아무것도 막지 못하고**, 앵커도 침묵한다(`txnId ≤ highest` — 정정 132ⓐ 가 설계상 그렇게 만들었다) | **ref 예외 조항을 못박는다**: 「귀속이 없어도 그 txnId 의 결과 ref 가 `refs` 입력에 존재하면 `never-applied` 를 답하지 않는다(→ reconciliation)」. 정정 150 이 이미 두 열거를 시그니처에 실었으므로 **추가 입력 없이 구현 가능**하고, 앵커가 원리적으로 침묵하는 구간을 정확히 덮는다. **PR3c 귀속**(판정 함수와 함께 이동) |
| 164 | **활성 저널 집합이 이중 정의**됐다 — T13a ⓓ 는 stage 단독 순수 술어인데 ⓕ 는 「귀속 필요」로 재정의했고, 소비자 4개가 전부 PR3b 밖이라 실제 게이트는 **stage 단독**을 쓴다 → 정정 154ⓐ 가 닫았다는 영구 고착이 **`delete-record` 표면에서 재현**된다. 그리고 `never-applied` 를 표현할 **어휘·전이·주체가 전부 없다**(`IntegrationStage` 5값·`AbandonReason` 3값 어디에도 없다) | 활성 판정을 **함수 1개**로 통일하고 **같은 커밋에서** spec:1093 I11 · spec:1203 T33 문면을 개정한다(정정 153 이 세운 규율의 적용). `never-applied` 는 **어휘 신설이 필요**하므로 **PR3c 로 이동**(판정과 함께). ⚠ 회수는 **삭제가 아니라 기록**이며 §W-7:778 「복구 중 삭제 금지」에 걸리지 않는다는 것을 **정정 135 의 보존 규율 인용으로 문면에 남긴다**(지금은 그 한 줄이 없어 구현자가 셋 중 무엇을 해도 계약 위반이다) |
| 165 | 축소된 PR3b 에 남는 3건 | ⓐ**저널 배치를 문법과 함께 못박는다**: `<area>/journal/<benchId>/<txnId>.json`(spec:160) · tmp = **같은 디렉터리**의 `<txnId>.json.<ownerToken>.tmp` + 계약 테스트 「benchId 디렉터리 밖에 어떤 저널 파일도 만들지 않는다」 1행(없으면 flat 배치가 정정 155ⓐ 결함을 재생산하고 PR3d 착수 전까지 안 드러난다) ⓑ**spec:161 의 별도 `tmp/` 라인을 폐기**한다 — tmp 는 대상과 같은 디렉터리에 만든다(착지 코드가 이미 그렇다). authority.ts:107-108 주석 정정(정정 133·152·156)과 **같은 커밋**에 묶으면 비용 ≈ 0 ⓒ**vacuous 계상 목록을 7건으로 갱신**(기존 5 + 규칙 1 행동 단언 + 저널 열거의 생산자 부재) |

#### Codex 리뷰 3라운드 반영 (2026-07-30) — ⚠ **「무응답」은 내 관측 오류였다**

**Codex 는 세 번 다 응답했고 첫 응답은 게시 7분 뒤였다**(`issuecomment-5117404144` · `…874859` ·
`…9265470`). 「2시간 15분 무응답」은 **내 폴링의 페이지네이션 누락**(코멘트 115건 중 최신 15건이 2페이지에
있었는데 1페이지만 조회)에서 나온 오판이고, 그 위에 세운 「제목 안 멘션은 트리거 안 됨」·「👀 소멸 =
처리 실패」도 전부 거짓이다. 로컬 적대 반증 자체는 값이 있었으나(결정 2건을 BROKEN 으로 뒤집었다)
**전제가 거짓이었고 그 사이 Codex P1 3건을 방치했다.** 세 라운드 전부 `Changes requested`.

| # | Codex 지적 | 내 로컬 라운드와의 관계 | 확정 |
|---|---|---|---|
| 166 | **P1 — `max` 앵커는 같은 ms 후속 시도를 놓친다.** 반례가 내 것보다 **직접적**이다: 시계 역행이 아니라 **같은 ms 안에서 T2 가 T1 보다 작게 생성되면 `max` 가 애초에 전진하지 않는다**(정정 149 는 시계 역행 경로만 봤다). 처방도 더 강하다 — ULID 사전순 워터마크를 **쓰지 말고 명시적 단조 세대**를 도입하라 | 로컬 2차와 **독립 수렴**(같은 결함) · **처방은 Codex 가 우위** | **정정 149 의 「strict 전진 + ULID 클램프」를 폐기하고 Codex 안 채택**: `highestIntegrationGeneration`·`currentIntegrationGeneration`(prepared 마다 `+1`) · 결과 ref 문법에 **고정폭 세대 결속**(`…/<benchId>/<fixed-width-generation>-<txnId>`) · ULID 는 **고유 식별자 역할만**, 순서 역할 없음. 전이 술어 4개 강제(퇴행 금지 · prepared 는 정확히 +1 · current == 워터마크 · journal/ref 세대 일치). ⇒ 정정 146·149·153 을 파생시킨 **ULID 정렬 의존 축이 원천 소멸**한다 |
| 167 | **P1 — journal-ahead 를 「항상 커밋 전 크래시」로 일반화 금지.** 권위 CAS 는 크래시 없이도 6가지 이유로 거부된다 | 로컬 3차 A1·A2 와 **같은 축**이나 Codex 가 **복구 분류를 2종으로 나눴다** | 저널 **선기록 전에** 같은 리스 구간에서 ①fresh read ②expected revision·txn·stage 확인 ③next draft 생성 ④단일 레코드 불변식 ⑤**신·구 전이 불변식** 검사를 **전부 끝낸다**. 저널에 `expectedAuthorityRevision`·`previousAuthorityStage`·`nextAuthorityStage`·`integrationGeneration`(+draft digest)을 **결속**. 복구는 **자동 승격 가능**(expected revision·stage 일치 ∧ 전이 여전히 유효 ∧ 미-superseded ∧ 리스 재획득)과 **승격 불가**(CAS 가 이미 `invariant-violation` · 기대값 불일치 · superseded · digest 불일치 · 버전 스큐)로 **2분**하고, 후자는 「도달 불가 stage 를 정상 대기로」가 아니라 **`reconciliation-required` 실패 기록**이다 |
| 168 | **P1 — 3표면에 단일 ownerToken 규칙 적용 불가.** `active-instance` 발행 tmp 는 **bench 리스 취득 *전*** 에 만들어진다(인스턴스 배타를 획득하는 과정 자체) → bench 리스 보유자가 그것을 「다른 token = 죽은 잔재」로 지우면 **살아있는 발행을 파괴**한다 | 로컬 2차와 **독립 수렴** · Codex 가 **`active-instance` 축을 추가**(내 2차는 `.area.json` 만 봤다) | **삭제 권위를 최소 2종으로 분리**: ⓐ`authority`/`journal` tmp = **리스 소유 수확기**(이름에 `benchId + 실제 writer ownerToken` · 자기 token 은 삭제 금지 · journal 은 repo 락과 bench 리스 중 **무엇을 요구하는지 명시**) ⓑ`coord`/`active-instance` tmp = **bench 리스 기반 수확 전면 금지** · active-instance 배타를 **성공한 단일 인스턴스만**, 파일명 문법·create-only 상태 검증 후 회수하는 **별도 계약**. 정정 155 의 「2표면」 축소는 유지하되 근거를 이 분리로 교체 |
| 169 | **P1(3R) — `revision` 앵커는 「현재 txn 1건」이 아니라 보존된 *모든* 결과 ref 를 비교해야 한다.** 시나리오: T1 이 revision 10 으로 발행 → abandoned 되어 current 슬롯에서 빠지지만 ref 는 **보존** → authority 가 14 까지 전진 → file-only 크래시로 **8 로 롤백** → current 만 검사하면 revision 10 ref 를 못 보고 새 CAS 허용 → **revision 9·10 이 재사용**돼 기존 세대와 충돌 | 로컬 3차 A5 가 제안한 `revision` 앵커안의 **성립 조건을 확정**해 줬다(순환 아님) | 비교 정의역 = **보존된 전 결과 ref**(`published`·`abandoned`·`superseded`·`finalized` 전부) · **앵커 판정이 어떤 새 CAS 보다 먼저**(부팅·재획득 후 fresh read → 전수 열거·검증 → `refRevision > authority.revision` 이 1건이라도 있으면 fail-closed) · **열거 실패도 fail-closed**(git 실패·문법 오류·스키마 불일치·D/F 충돌은 「결과 없음」이 아니다) |
| 170 | **`unavailable` 소멸 주장은 「좁은 의미로만」 참**(3R) | 로컬 3차 A3 의 결론을 **정확히 좁혔다** | 문면 확정: 「**PR3b/PR3c 배치 불일치만을 위해 존재하던 영속 `anchor-unavailable` 상태는 제거한다. 다만 authority/ref 관측 실패는 별도 fail-closed 오류 또는 reconciliation verdict 로 유지한다**」(권위 read 실패·버전 초과·identity mismatch·ref 열거 실패·잘못된 ref 문법·namespace 충돌·권위 부재+잔존 ref) |
| 171 | **승인된 것**(3라운드 공통) | — | **PR3b/3c/3d 분할 경계 명시 승인**(「revert 축이 명확히 다르다」) · `prepared` 시점 선갱신 순서 · 신·구 전이 불변식 계층 필요성 · `abandoned` 감사 보존(단 열거 시 **7종 검증 필수**: `.json` 정확 접미 · basename ULID 문법 · 내부 `txnId` == basename · `benchId` == 상위 디렉터리 · `schemaVersion` 최우선 · **regular file 만**(symlink/FIFO 는 읽기 전 거부) · tmp 접미는 저널로 읽지 않음) · `delete-record` 후 고아 ref 는 **repo-level orphan/tombstone**(revision 0 재생성 금지) |
| 172 | **정정 156 의 사실 다툼 — 내 소스 근거가 맞다** | Codex 2R 이 **git 문서**를 근거로 「`reference` 가 기본에서 빠진다」를 반박했다. 1R 에서는 「HTTP 401/403 으로 외부 문서 조회 실패 · 독립 확인 못 함」이라고 정직하게 적었다 | **git 소스 v2.54.0 직접 확인으로 확정**(메인 루프): `FSYNC_COMPONENTS_DEFAULT ((FSYNC_COMPONENTS_OBJECTS｜FSYNC_COMPONENTS_DERIVED_METADATA) & ~FSYNC_COMPONENT_LOOSE_OBJECT)` = `PACK｜PACK_METADATA｜COMMIT_GRAPH` → **REFERENCE 부재**. `FSYNC_COMPONENTS_COMMITTED (FSYNC_COMPONENTS_OBJECTS｜FSYNC_COMPONENT_REFERENCE)` 는 REFERENCE 를 포함하지만 **런타임 기본값은 `committed,-loose-object` 가 아니라 `DEFAULT` 다** · `FSYNC_COMPONENTS_PLATFORM_DEFAULT` = `DEFAULT`(오버라이드 없음). 즉 **git 문서가 코드와 어긋나 있고** Codex 2R 은 그 문서에 근거했다 — 내 FSYNC 에이전트가 「문서만 읽으면 정반대 결론이 나온다」고 **예고한 그대로**다. ⚠ 그러나 **조치는 양쪽이 동일**하다: `authority.ts:107-108` 의 「동시 롤백이 불가능하다」는 **즉시 삭제**하고 「보조 앵커 · 완전한 rollback detector 아님 · 동시 롤백 시 fail-open」으로 재기술한다 |

**Codex 반영 후 PR3c 의 형태가 바뀌었다**: 앵커는 `highestIntegrationTxnId`(ULID 순)가 아니라
**명시적 단조 integration generation**이고, 결과 ref 문법에 그 세대를 결속하므로 **C1 문법 변경이 확정
사항**이 됐다(3차에서 「PR3c 이월 열린 결정」으로 남겼던 것이 Codex 3라운드로 닫혔다). PR3b 는 여전히
이 결정들에 **의존하지 않는다** — 다만 정정 167 이 저널 레코드에 4필드(`expectedAuthorityRevision` 등)를
추가하므로 **PR3b 의 레코드 타입에 그 필드가 포함**된다.

**3차 라운드 결론 — 축소된 PR3b 는 미결 결정이 0 이다.** 남은 것은 전부 이번 라운드에서 **실측으로
확정된 것**뿐이다: 레코드 타입·`schemaVersion` 최우선 파서·조건부 불변식·직렬화·내구 쓰기(win32
rename EPERM 재시도 실측 완료)·tmp 문법과 배치·열거 필터(접미+ULID 검증 실측 완료). **앵커·복구 판정·
회수 어휘는 전부 PR3c 로 이동**했고, 그것들의 미결 쟁점은 **PR3c 자신의 착수 전 라운드**가 다룬다.

> **PR3c 착수 전 라운드로 이월된 열린 결정 1건**(3차 A5 제기 · 이번에 결정하지 않는다): 앵커의 정렬
> 키를 ULID 사전순 대신 **권위 `revision`** 으로 두는 안 — 결과 ref 이름에 revision 을 실어
> (`…/<benchId>/<revision>-<txnId>`) `record.revision` 과 비교한다. `revision` 은 authority.ts:104-110 이
> 「CAS 성공마다 정확히 +1」로 **단조를 이미 집행**하므로 시계 역행 계열(정정 146·149·153 세 정정을
...
**T69**(크레덴셜 타입 강제) · **T70**(리스 아래에서만) · **T71**(불변 필드·결속) · ~~**T72**(생성 저널
3채널)~~. 기존 §3 행 중 PR3b 가 **non-vacuous 로 소비**할 수 있는 것은 **T62 의 D-9 저널 분 1건**뿐이라는
것이 정찰의 실측이고, 그래서 신설이 유일한 충족 경로였다.

#### PR3b 착지 실측 (2026-07-30)

| # | 실측 → 판정 | 조치 |
|---|---|---|
| 181 | **분량 프록시가 이 PR 에서 크게 빗나갔다.** 착수 트리거는 「프로덕션 물리행 ≤400」(= 1,900 ÷ 최악 배수 4.75)이었는데 `journal.ts` 는 **838 물리행**으로 그 2.1배다. 그런데 **순증은 1,840** 으로 상한(1,900) **안**이다 — 실측 배수가 **2.20** 이라 정정 139 가 6 PR 로 세운 밴드(2.69~4.75)의 **하한마저 밑돈다**. 원인은 구성비다: 이 PR 은 주석 밀도가 높은 **순수 모듈 1개 + 테스트 1개**라 「프로덕션 1행이 테스트·부수 변경을 몇 행 끌고 오는가」가 형제 PR 들보다 훨씬 작다(형제들은 착지 계약 개정이 여러 파일로 번졌다) | ⓐ밴드를 **2.20~4.75** 로 갱신한다(7 PR 전수). ⓑ**트리거를 「물리행」에서 「프로덕션 물리행 중 비주석 행」으로 바꾸지 않는다** — 세 번 빗나간 「행 개수 × 행당 비용」 모델로 되돌아가는 것이기 때문이다. 대신 **프록시가 보수적으로 틀릴 수 있음을 명시**하고, 착수 트리거 초과 시 「즉시 분할」이 아니라 **순증 실측으로 재판정**한다. ⓒ⚠ **1,840 은 상한의 97%** 다 — 리뷰 라운드가 코드를 늘리는 축(정정 74)이 남아 있어 **머지 시점 초과 가능성이 실재**한다. PR3b 는 사전 분할점이 「없음」이므로 초과하면 은폐하지 않고 PR 본문에 재측정값으로 기록한다 |
| 182 | **T72(생성 저널 3채널 판정)를 PR3b 가 착지시키면 정정 159 의 안티패턴이 되살아난다** — 판정 규칙의 소비자·생산자가 **둘 다 PR4 T16** 이고, 대상 레코드도 통합 WAL 이 아니라 **생성 저널**(다른 레코드 타입)이다. 즉 「판정 함수가 자기 입력의 생산자보다 먼저 착지」 + 「이 모듈의 계약이 아닌 것을 이 모듈이 소유」가 겹친다 | **§3-T72 귀속을 PR4 T16 으로 이관**하고 규칙·행동 단언을 **한 커밋에서** 착지시킨다. 정정 143 은 「규칙만 먼저」였으나 그것은 정정 159 **이전** 판단이다 — 같은 기준을 일관 적용한다. PR3b 의 §3 신설은 **8행**으로 확정 |
| 184 | **Codex PR#269 이 P1 5건을 냈고 전부 실재했다** — ⓐ주장한 `previousAuthorityStage` 가 디스크와 **대조되지 않아** 저널이 결속 없던 전이를 증언할 수 있었다 ⓑ`abandoned → abandoned` 무조건 허용이 **종결 증거를 다른 사유·시각으로 덮어썼다**(파일이 하나라 원본이 그 자리에서 소멸) ⓒrevision 결속만으로는 **같은 크레덴셜로 두 전이**를 써 저널이 권위를 앞섰다 ⓓ백오프가 이 모듈의 첫 `await` 라 호출자가 `await` 하지 않으면 **뮤텍스가 풀린 뒤에도 쓰기가 살아남는다**(그때 리스는 유효해 L-6 로 못 막는다) ⓔ`mkdirRecursive` 가 **심링크를 따라가** 영역 밖에 쓸 수 있었다 | 전부 반영(`41615aa`). ⓐ`previousAuthorityStage === from` 강제 ⓑ**정확한 멱등 재생만** 허용(전체 정준 비교) ⓒ**저널 지역 원장**으로 「크레덴셜 1개 = 전이 1개」(형제 `SPENT_COMMITS` 는 런처가 소비하므로 공유 금지 · 정확 재시도는 허용) ⓓ`sectionLive` **필수 주입** + `section-closed` 종별 신설(`sleep` 과 같은 근거로 기본값 금지) ⓔ`PathKind` 에 `'symlink'` **추가**(읽기 측 소비자는 `!== 'regular'` 비교라 계약 무변경) + mkdir 전 거부. **착지 계약 변경분이 하나 늘었다** — 「추가 export 1」에서 「추가 export 1 + `PathKind` 확대 1」로 PR 본문을 정정한다. ⚠ **그 확대가 `describe.skipIf(win32)` 안의 기존 단언을 깼고 CI(ubuntu)만 잡았다** — 로컬 GREEN 이 CI 를 증명하지 않는 축에 **이 PR 이 스스로 경고해 놓고** 반대 방향으로 걸렸다(교훈: 공유 타입을 넓히면 그 타입의 **플랫폼 게이트 뒤 단언**을 전수 확인한다) |
| 185 | **정정 175ⓓ 를 번복한다** — Codex 2R P1: 내가 넣은 「크레덴셜 1개 = 전이 1개」 원장이 **객체 동일성** 키라, `{...commit}` 복제가 브랜드를 보존한 채 새 객체로 통과해 원장을 **빈 채로 조회**하게 만든다(정품으로 `composed` → 복제본으로 `published` → 저널이 권위를 두 단계 앞선다). 175ⓓ 는 「착지 계약 무변경」을 근거로 `isMintedCommit` 신설을 기각하면서 「필드를 믿는 검사는 위조 방어가 아니라 스레딩 사고 방지」라고 적었는데, **그 문면이 곧 이 결함의 자백**이었다 | `authority.ts` 에 **`isMintedCommit`·`isMintedRead` 추가 export**(형제 `isMintedLease` 와 동형 · **조회일 뿐 소진 아님** — 소진은 런처 계약이고 저널이 함께 소비하면 「CAS2 → 저널 → spawn」이 불가능해진다). 출처를 먼저 보면 **위조 조립까지 함께 닫히고** 원장의 동일성 키 전제가 비로소 성립한다. ⚠ **교훈**: 「착지 계약 무변경」을 지키려고 방어를 약화시키면 그 약화가 다음 라운드의 P1 이 된다 — 계약 변경 비용과 방어 강도를 저울질할 때 **약화 쪽 근거를 문면에 적고 있다면 그것이 이미 신호**다(PR#266 이 `FreshReadToken` 에서 실측한 벡터의 저널 표면 재현) |
| 186 | **Codex 3R P1 6건** — ⓐ「`schemaVersion` 최우선」을 **문면으로만** 지키고 있었다(오염 키 검사가 먼저라 상위 버전 파일이 `invalid` 로 분류됐다 — `invalid` 는 broken·삭제로 흘러갈 수 있고 `incompatible-version` 은 그것을 막아야 하는 종별이다) ⓑ옵션이 **호출자 객체 그대로**라 `journalDir` 를 나중에 바꾸면 쓰기 경계가 재조준된다(`readonly` 는 컴파일 타임 표기다) ⓒ크레덴셜 원장·겹침 가드가 **store 지역**이라 두 번째 store 를 만드는 것만으로 우회된다 ⓓ읽기 증거가 **민팅 조회만** 통과해 소진된 토큰·닫힌 구역의 토큰이 활성 WAL 증거를 남긴다 ⓔ다이제스트가 **호출자 draft 를 통째로** 해시해 CAS 가 기록할 투영과 어긋난다 ⓕ**결과 증거가 단계 전진에서 교체 가능**했다(`composed → published` 가 `resultOid` 를 바꿔치기하면 원본이 소멸한다) | 전부 반영(`969bfd6`). ⓐ버전 판정을 최상위 객체 확인 직후로 · ⓑ생성 시점 `Object.freeze` 스냅숏 · ⓒ**모듈 스코프**로 이동(형제 정정 95 를 **읽고도 같은 실수를 했다**) · ⓓauthority 에 **`isLiveUnspentRead`** 신설(민팅 ∧ ¬소진 ∧ **발급 구역 생존** — `LIVE_SECTIONS` 로 클로저 지역 `live` 를 모듈 밖에 노출) · ⓔauthority 에 **`projectAuthorityDraft`** 신설(`serialize` 를 그대로 거치므로 필드 목록이 갈릴 수 없다) · ⓕ**한 번 나타나면 동결**(처음 등장은 허용 — 불변 12필드와 다른 목록). ⚠ ⓓ가 **테스트 하네스를 바꿨다**: 「미리 뽑아 둔 토큰을 나중에 쓰는」 형태가 무효가 되어 41곳을 **구역 안에서 append 를 부르는** 형태로 재작성했고, 그것이 실제 WAL 순서와 같아 이전 하네스보다 프로덕션에 가깝다. **교훈**: 형제가 이미 문면으로 남긴 규율(정정 95 의 「store 지역 금지」)을 **읽고도** 같은 형태를 다시 만들었다 — 신설 모듈에서는 형제의 **원장·스코프 결정을 체크리스트로 대조**한다 |
| 187 | ⚠ **범위 재확정(사용자 결정) — 크레덴셜·임계 구역 결속 축을 PR3c 로 이관한다.** Codex 4라운드가 **P1 16건**을 냈고 전부 실재했는데, 그중 다수가 **직전 라운드에서 내가 넣은 방어 자체**의 구멍이었다: 원장 부재 → 원장의 **동일성 키** 우회(`{...commit}`) → 원장의 **store 스코프** 우회 → capability **미인증**(`sectionLive` 가 평범한 클로저) → 크레덴셜이 **관측 txn 을 증언하지 않음**(T2 커밋으로 T1 전진). 남은 처방은 **authority 계약 확장**을 요구한다 — 구역 capability 민팅 + 크레덴셜에 관측 통합 상태(txn·stage·세대) 결속 — 그리고 그 필드 계약은 **PR3c 소유**, 유일한 소비자(시퀀서)는 **PR5** 다 | **PR3b = 레코드·파서·전이 술어·내구 쓰기·열거 필터**(+ 리스 인가·심링크 거부·증거 동결·스냅숏·버전 우선순위). `append(lease, draft)` 로 좁히고 크레덴셜·`sectionLive`·`section-closed`·원장·`isMintedCommit`/`isLiveUnspentRead`/`LIVE_SECTIONS` 를 전부 되돌린다(§3-T69 도 PR3c 로 재문안). **근거는 정정 159 와 동일**하다 — 「집행이 자기 입력의 생산자보다 먼저 착지하는 배치」가 문제였고, 그때도 방어를 더 얹는 대신 배치를 고쳤다. ⚠ **분량이 근거가 아니다**(이관해도 상한 밑으로 못 내려간다) — 근거는 **한 PR 이 지는 불변식 표면**이다 |
| 188 | **「PR3b 실측·라이브 테스트가 더 필요한가」에 대한 판정**(사용자 질문 · 머지 후) — ⓐ**실측은 이미 프로덕션 조합으로 돌고 있다**: `journal-node.test.ts` 는 페이크 0(실 `createNodeDurableFs` + 실 `mkdtemp`)이고 CI 3조합(ubuntu/22 · **windows/22** · ubuntu/**24 출하 런타임**)에서 전부 실행된다 — win32 C4 는 `openSync` 로 **진짜 EPERM** 을 유발해 재시도 커밋까지 본다 ⓑ**라이브(dogfood)는 원리적으로 불가능하다**: `journal.ts` 의 프로덕션 임포터가 **0**(테스트 2개뿐)이고 `probeDurability`·`openCoordinationArea` 도 호출자가 0이라 Electron 을 띄워도 도달하는 콜스택이 없다 | **PR3b 단계에서 dogfood 를 하지 않는다** — 배선 전 모듈에 라이브를 걸면 「0줄 실행 → GREEN」이라는 가장 나쁜 위양성이 된다. 라이브의 정당한 시점은 소비자가 붙는 **PR5/PR7**. ⚠ 로컬 win32 에서 「실측 돌렸다」는 관측은 **POSIX 3건(모드·실 심링크·실 dir fsync)에 무신호**다(`describe.skipIf` · 이 머신은 `symlinkSync` 가 EPERM 이라 물리적으로도 못 돈다) — 그 축의 관측면은 ubuntu CI 잡 **둘**이며 거기서 실제로 돈다 |
| 189 | ⚠ **리뷰가 나중에 넣은 방어가 기존 뮤테이션 자기검사를 가렸다** — 정정 183 의 스윕(30 뮤턴트) **이후** 정정 184ⓐ가 `previousAuthorityStage !== from` 을 전이 게이트 **바로 아래**에 넣었고, 두 검사가 **같은 종별**(`invariant-violation`)을 돌려주는데 기존 게이트 테스트 2개가 **종별만** 단언해서 가려졌다. 실측: `if (!canAdvanceStage(...))` 를 `if (false)` 로 통째 무력화해도 104건 전부 GREEN. 같은 축에서 `0o700/0o600 → 0o777/0o666` 도 GREEN 이었다(`args[0]` 만 봐서 mode 인자 무신호 — 형제 `authority-store.test.ts:1182` 는 이미 `toEqual([path, mode])`) | 테스트 전용 수정: ⓐ게이트 행들의 `previousAuthorityStage` 를 **디스크와 정직하게 맞춰** 결속 검사를 통과시키고 **위반 메시지**까지 단언 ⓑ미겨냥 축 2행 신설(**부재 → published** = WAL 선기록 우회 · **자기 전이** `composed → composed`) ⓒmode 인자 전량 단언. 재실측: 두 뮤턴트 **RED**(4건·1건 실패), 기존 18방어 + 이웃 2개(결속·포기 재생) **전부 잡힘**. ⚠ **교훈**: 뮤테이션 자기검사는 **그 시점의 방어 집합**에 대한 측정이다 — 리뷰 라운드가 방어를 추가하면 그 라운드 뒤에 **다시 돌려야** 한다. 새 방어가 기존 방어와 **산출 종별을 공유**하면 가림이 기본값이고, 그래서 종별이 아니라 **메시지**를 단언한다 |
| 183 | **뮤테이션 자기검사가 단언 공백 5건을 찾았다**(30 뮤턴트 · 29 잡힘) — 그중 둘은 **실재 결함**이었다: ⓐ리스 identity 대조에서 **benchId 축만 단언 부재**(뚫리면 A 의 리스로 B 의 디렉터리에 쓴다 — 경로가 draft 에서 유도되므로) ⓑ`Omit` 의 초과 프로퍼티 검사가 객체 리터럴에만 걸려 **draft 가 `schemaVersion` 을 실어 올 수 있었다**(형제 정정 73 동형 · 스프레드 순서 때문에 store 배정을 덮어썼다) | 둘 다 코드·단언으로 폐쇄(스탬프를 스프레드 뒤로 + 키 존재 자체 거부). **후속 규율**: 이 슬라이스에서 「전 게이트 GREEN + 첫 구현이 전 테스트 통과」는 **테스트 공백의 신호로 취급**한다 — 뮤테이션 1회차 3건 생존이 그 실증이다. 도달 불가로 확인된 방어 2건(왕복 파싱 · 배열 정준화)은 **삭제**했다(도달 불가 arm 은 커버리지에 안 잡히면서 「검증한다」는 인상만 준다) |

#### PR3c 착수 전 라운드 (2026-08-12 · 계획:97-99·1136 이 요구한 미결 결정 닫기)

계획 표:92 가 PR3c 의 분할점을 「착수 전 라운드에서 재도출」로 남겼고, 정정 166·169·149 가 **서로 다른
곳에서 각각 「확정」된 세 벌의 앵커 문안**으로 남아 있었다. 여기서 하나로 접는다.
...
**`refRevision = expectedAuthorityRevision`**(정정 196 이 off-by-one 으로 폐기). ⚠ **이 폐기의 정의역은
`composed` 저널이다**(정정 223) — stage 별 식에서 `published`(stageIndex 2)의 **정상** 결속은 정확히
`refRevision = expected` 이므로, 이 줄을 문자대로 집행해 그 등식을 「폐기 어휘 재유입」으로 읽으면 안 된다.
...
fail-closed(⚠ **stage 마다 정의역이 다르다** — `<N>`(= expected 그대로)은 `composed`·`prepared` 에서 위반이지만
`published` 에서는 **정상**이고 `finalized` 는 상한이라 통과한다 · 정정 223·233) · **T76** 보존된 **전** 결과 ref 에 동일 판정 적용 · **T77** 권위 부재 + ref 잔존 = revision `0`
...
#### PR3c 착지 실측 (2026-08-12 · 구현 중 확정 3건 + 뮤테이션 자기검사)

**분량** — ⚠ **아래 초기 수치는 stale 이었다**(로컬 적대 리뷰 L5 가 지적 · 정정 139 가 세운
「푸시 시점 수치를 최종으로 적지 않는다」 규율의 4회차 재발). 라운드 반영 8커밋이 더 들어간
**현 head 기준 재측정**이 권위다:

| 시점 | 순증(src) | 프로덕션 | 상한 1,900 대비 | 목표 ≤400 대비 |
|---|---:|---:|---:|---:|
| 착지 직후(초기 기록) | 2,031 | 744 | +7% | +86% |
| **현 head(리뷰 7라운드 반영 후)** | **3,469** | **1,067** | **+83%** | **+167%** |

프로덕션 내역: `recovery.ts` 718 · `authority.ts` +208 · `result-ref.ts` 118 · `journal.ts` +25.
**은폐하지 않는다**(PR3b 정정 181 과 같은 규율). 초과의 성격은 「기능 추가」가 아니라 **리뷰 7라운드가
요구한 방어와 그 정직 표기**다 — Codex P1 23건 + 로컬 P1 1·P2 3 + 테스트 반증력 14건이 전부 코드·주석·
테스트로 착지했다. ⚠ 최종 수치는 머지 직후 `origin/master` 기준으로 다시 재측정해 갱신한다.

⚠ **분할 재검토를 하지 않은 이유를 여기서 다시 적는다**(초과가 2배를 넘었으므로): 정정 233 이 확정한
결속 축은 **정정 215·218·223 을 되돌리는 수정**이라 그 세 정정과 같은 revert 단위여야 하고, 그것들은
이미 이 PR 안에 있다. 새 PR 로 빼면 「정상 상태가 영구 reconciliation 되는 코드」가 master 에 한 커밋
동안 존재한다 — PR0 이 확립한 「유일한 비가역 ripple 이 자기 revert 단위를 갖는다」의 정반대다.

**추가 분할을 하지 않은 근거**: 정정 159·161 이 세운 축 논거가 그대로 유효하다 — 앵커 필드·전이 불변식
계층·판정 함수가 **한 커밋에서 함께 태어나지 않으면** `anchor-unavailable` 유령 상태가 되살아난다.
분량은 분할 근거가 아니고 **한 PR 이 지는 불변식 표면**이 근거라는 정정 187·195 의 기준을 적용하면
이 PR 의 표면은 여전히 하나다(크레덴셜 축은 이미 PR3c′ 로 나갔다).

| # | 구현 중 확정 | 근거 |
|---|---|---|
| 205 | **앵커를 `durability` 값으로 게이트하지 않는다** — 판정은 **항상** 돈다 | 발화가 필요한 표면은 `file-only`(C3)이지만 그 값은 **롤백 대상인 권위 파일 안에** 있다(`writtenBy.durability`). 검사의 **활성화 조건**을 검사 대상과 같은 매체에서 읽으면 독립성이 그 지점에서 조용히 끊긴다 — 2R 교훈(「면제 규칙을 권위 파일에서 읽어오면 독립성이 끊긴다」)의 **동형**이고, T13b 문면의 「발화 조건은 `durability === 'file-only'`」는 그래서 폐기한다. `file+dir` 표면에서는 정상 프로토콜상 **발화 입력 자체가 만들어지지 않는다** |
| 206 | **면제 조건 ⑩(promotable 정확히 하나)은 이 PR 에서 독립 반증력이 0 이다** — 남기되 **정직 표기** | 뮤테이션 자기검사 실측: `promotable.length === 1` 을 `>= 1` 로 바꿔도 **테스트가 잡지 못한다**(16 뮤턴트 중 유일한 생존). 조건 ⑧(전이 불변식)이 「`composed` 는 새 txn 을 시작할 수 없다」를 강제해 통과 후보가 **현재 txn 하나**로 이미 좁혀지기 때문이다. 정정 183 의 「도달 불가 arm 금지」와 정면으로 부딪히지만 **삭제하지 않는다** — ⑩ 은 Codex 가 승인한 계약 조항(정정 199ⓒ)이고 ⑧ 이 약해지는 미래 편집에서 마지막 방어다. 대신 **「가림」을 코드 주석·PR 본문에 계상**하고, §3-T88 의 **관측 가능한** 계약은 「면제되지 않은 모든 후보가 blocker」라는 루프가 진다는 것을 명시한다 |
| 207 | **고아 활성 저널 2분의 조작화**(정정 194 의 문면을 값으로) — benign = `stage === 'prepared'` **∧** 그 txn 의 결과 ref 부재 | (A) 프로토콜에서 「권위가 가리키지 않는 활성 엔트리」가 정상일 수 있는 창은 **하나뿐**이다 — 저널 선기록 후 `prepared` CAS 전. 그 창의 엔트리는 결과 ref 를 **가질 수 없다**(ref 는 composed 저널 뒤에 발행). 따라서 `composed` 고아·ref 를 동반한 `prepared` 고아는 전부 reconciliation 이고, 활성 집합의 **정의**는 stage 단독으로 유지된다(정정 194 가 요구한 방향) |

**이 PR 에서 vacuous 한 것**(사전 계상 · 머지 직전 적발을 만들지 않는다): §3-**T80**(복구기가 인가를
차단하는 **행동**) · **T81**·**T92**(락·리스 아래 fresh 재검증의 **행동** — 여기 있는 것은 증거 봉인
seam 과 동치 판정뿐) · **T91**(변이 순서 — 판정 함수가 **순수**하다는 사실이 그 구조적 형태다) ·
**T93**·**T94**(CAS 성공·실패 후 상태) · §3-**T18b** 후반 연언(「되돌아간 revision 위에 새 CAS 가
커밋되지 않는다」). 전부 **생산자가 PR5(시퀀서)·PR7(부팅)** 이며 그 사실을 스펙 행 문면에도 적었다.

**뮤테이션 자기검사**: 16 뮤턴트 중 **15 잡힘**. 생존 1건이 위 정정 206 이다. ⚠ 첫 실행에서는 **2건이
생존**했고 그중 하나(면제 조건 ⑥ 산술 결속 삭제)는 **실제 테스트 공백**이었다 — 「⑥ 위반」을 검증하던
픽스처가 `refRevision ≤ revision` 이라 **복구표 경로**로 빠져, 면제 조건 자신의 반증력이 한 번도
관측되지 않았다. 후보 경로(`refRevision > revision`)용 행을 추가해 닫았다.

**교훈 — 제어문자 오염이 착지했고, 그 진단을 내가 두 번 틀렸다.** `recovery.ts` 의 템플릿 리터럴 안에
U+0000 이 섞였고 발견 경로는 게이트가 아니라 `grep` 이 그 파일을 「binary file」로 답한 것이었다.

⚠ **여기서 내가 「`npm run verify` 는 NUL 을 보지 않는다」고 적었는데 그것은 거짓이다**(아래 정정 214
에서 실측으로 반박). `scripts/repo-hygiene.test.ts` 의 「원시 NUL 0건」 게이트가 **이미 존재하고
실제로 잡는다** — 다만 나는 grep 으로 먼저 발견해 **고친 뒤에** 전체 게이트를 처음 돌렸으므로 그
게이트가 발화할 기회가 없었다. 「내가 먼저 찾았다」를 「게이트가 못 잡는다」로 읽은 것이 오진이다.
진짜 구멍은 **NUL 외 C0/C1 제어문자**이며 그것은 정정 214 가 실측으로 확정한다.

#### PR#289 Codex 리뷰 1R 반영 (2026-08-12 · P1×5 · **5건 전부 실재 · 전부 수용**)

**두 건이 「다른 층이 막는다」는 내 검증 안 된 주장이었다**(정정 133·152·156 이 닫은 것과 **같은 형태의
재발**). 나머지 셋은 관측 계약의 구멍이다.

| # | 지적 → 확정 | 근거 |
|---|---|---|
| 208 | **최초 레코드(`prev` 부재)도 시작 stage 를 본다** — `prepared` 외로 태어나는 첫 CAS 거부 | 내 주석은 「최초인데 미종결 통합을 들고 태어나는 것은 **단일 레코드 불변식 ③ 계열의 소관**」이라며 침묵했는데 **거짓이었다** — `checkInvariants` 의 ③·③b·③c·③d 어디에도 「최초 stage 는 prepared」가 없다. 첫 CAS 가 `composed` 로 태어나면 **WAL 1단계와 그 크래시 안전 순서를 통째로 건너뛴다.** ⚠ 파급으로 기존 스토어 테스트 5건이 깨졌는데, 전부 **첫 CAS 로 목표 stage 를 만드는 픽스처 지름길**이었고 그 지름길이 성립한다는 사실 자체가 결함이었다. `abandoned` 초기 레코드 기대는 **뒤집었다**(정정 177: 권위는 `abandoned` 를 갖지 않는다 — 이전 기대는 **어떤 생산자도 만들지 않는 상태**를 정상으로 고정하고 있었다) |
| 209 | **`completedIntegrationTxnId` 소거 금지의 예외 = 보관 전이** | 불변식 ②가 「존재 ⟺ `lifecycle==='integrated'`」이므로 `integrated → archived` 는 **반드시** 그 필드를 소거한다. 내 규칙은 그 정상 전이를 전부 `invariant-violation` 으로 답해 **보관 워크플로를 통째로 막았다**(착지 커밋 `b572a6c` 의 회귀 · 테스트가 0건이라 무신호였다). 예외는 「소거」가 아니라 **lifecycle 에 결속**시킨다 — 귀속 교체는 계속 거부 |
| 210 | **복구 판정의 identity 는 호출자가 독립으로 싣는다**(`benchId` → `BenchAuthorityIdentity`) | 초안은 레포 identity 를 **권위 레코드에서 꺼내** 저널과 대조했다. 권위 부재·손상이면 그 대조가 **통째로 사라져** 다른 레포에서 복사된 저널(같은 benchId · 영역 통째 복사)이 benign → `no-mutation` = 재준비 적격으로 통과한다. **정정 205 와 같은 논거의 동형**(검사 기준을 롤백 가능한 매체에서 읽지 않는다)인데 내가 같은 파일 안에서 한쪽만 적용했다 |
| 211 | **권위 stage ≤ 저널 stage ≤ 권위 stage + 1** 교차 결속 신설 | 복구표를 **저널 stage 만 보고** 분기했더니, 권위가 앞선 관측(= 저널 단독 롤백·손상)이 `no-mutation` 으로 통과했다 — 손상 위에서 포기·재준비가 인가되는 경로다. 두 부등식의 출처는 각각 WAL 선기록과 정정 204 불변식 ①이고, **두 방향을 다른 종별로** 답한다(진단 분해능 · 각각 독립 반증) |
| 212 | **고아 `prepared` benign 창은 「권위에 미종결 통합 없음」에서만 열린다** | 전이 불변식이 「미종결 T1 위에 T2 시작」을 금지하므로 T2 저널은 **애초에 쓰이지 말았어야 한다**. 그 존재 자체가 이상 신호인데 benign 규칙이 삼켰고, 그 상태에서 T1 의 `resume-composed-cas` 까지 그대로 나갔다 |
...
| 214 | **raw 제어문자 오염이 재발했다**(U+0001 · `sealEvidence` 구분자) — 이스케이프 표기로 치환하고, **기존 게이트의 실제 경계를 실측으로 확정**했다 | 발견자는 게이트가 아니라 **외부 리뷰어**였다. ⚠ 여기서 위 착지 실측 절의 「verify 는 NUL 을 보지 않는다」를 **반박**한다 — `src/main/core/workbench/__ctrl_probe.ts` 에 문자를 하나씩 심어 `scripts/repo-hygiene.test.ts` 를 돌린 결과: **U+0000 = RED(잡는다)** · **U+0001 · U+001F · U+007F = 통과(무신호)**. 즉 게이트는 **NUL 만** 보고 나머지 C0/C1 은 전부 통과시킨다. 구분자 자체는 **의도적으로 유지**한다(붙여 이으면 `["ab","c"]` 와 `["a","bc"]` 가 같은 다이제스트를 낸다) |

**후속 후보 갱신(#251 밖)** — **기존 NUL 게이트의 문자 범위 확장**(신설 스크립트가 아니다 · 정정 214 의
실측이 위치를 특정했다): `scripts/repo-hygiene.test.ts` 의 「원시 NUL 0건」을 **C0/C1 전반**으로 넓힌다
(허용 = 탭·LF·CR). 탐지식은 `[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F-\u009F]` 이고 스캔 루트·확장자는 기존 것을 그대로 쓴다.
⚠ 스캐너 자신의 패턴을 **이스케이프 문자열**로 쓸 것 — 문자 클래스에 raw 제어문자를 넣으면 그 스캐너가
같은 오염을 재생산한다(이 정정을 적는 편집이 실제로 8건을 재생산했고 스캐너가 잡았다).

**교훈(정정 214) — 「게이트가 못 잡는다」는 결론에도 관측 경로 검증이 필요하다.** 부정 결론(「무응답」)을
근거로 워크플로를 바꾸기 전에 관측 경로를 의심하라는 규율을 이미 학습해 놓고(Codex 무응답 오판 사고),
이번에는 **게이트의 무신호**에 같은 실수를 했다 — 「내가 먼저 찾았다」와 「게이트가 못 잡는다」는 다른
명제인데, 게이트를 **한 번도 그 상태로 돌려보지 않고** 후자를 단언했다. 문자별 프로브 실측은 5분이
걸렸고 결론을 정반대로 좁혔다.

**Codex 3R — P1×3 · 전부 실재 · 전부 수용**(2R 은 clean 이었는데 3R 에서 새 축이 나왔다).

| # | 지적 → 확정 | 근거 |
|---|---|---|
| 215 | **stage 가 같아도 결속을 본다** — 현재 txn 저널은 `pre-cas`(권위 = `previousAuthorityStage` ∧ expected === revision) 또는 `post-cas`(권위 = `nextAuthorityStage` ∧ revision === expected+1 ∧ 결과·세대 일치) **둘 중 하나**여야 한다 | 정정 211 의 순서 검사는 **인덱스만** 비교해서 「둘 다 composed 인데 resultOid·revision·세대가 어긋난」 손상 쌍이 복구표에 그대로 도달했고, ref 가 없으면 `no-mutation`(= 포기·재준비 적격)이 나왔다. ⚠ 이 결속을 세우면서 `classifyComposed` 의 `reflected` 재조립은 **삭제**했다 — 같은 조건을 두 곳에 적으면 한쪽이 무력화돼도 다른 쪽이 가린다(정정 189) |
| 216 | **새 txn 시작 인가를 `lifecycle` 에 건다**(stage 종결만으로는 부족) | `integrated` ∧ current `finalized` 레코드가 pending 검사를 통과해 **종결된 bench 를 다시 열 수 있었다.** 스펙 §5 「lifecycle 회귀(재개) 없음 · `integrated` 후 추가 작업은 **새 bench**」와 정면 충돌이고, 단일 레코드 불변식은 이것을 보지 않는다 |
| 217 | **완결 귀속의 「도입」도 검사한다** — 신규 `completedIntegrationTxnId` 는 직전 레코드의 current txn 이어야 한다 | 교체·소거 금지만 있어서 **첫 도입이 무제한**이었다: current 가 T1 인 레코드를 `completedIntegrationTxnId: T2` 로 커밋해도 단일 레코드·전이 검사가 **둘 다 통과**하고, 무관한 txn 이 완결로 기록되며 **부분 통합이 숨는다**. §W-8 의 「완결은 권위 시도만 · 관측 CAS 는 **txn 동일**」을 코드로 세운다. ⚠ 정직 표기: **도달성·stage 전제는 여기서 보지 않는다**(`resultOid` 의 base 도달성은 외부 관측이고 이 순수 함수의 입력에 없다 — §W-8 소관) |

**뮤테이션 3차**: 새 방어 6개(M24~M29) 중 **5 잡힘 · 1 생존**. 생존한 M26(`binding === 'post-cas'`
재검사)은 **도달 불가**임이 실측으로 드러나 **삭제**했다(도달성 증명은 코드 주석에 남겼다) — 정정 206
에서 「승인된 계약 조항이라 남긴다」고 판단한 ⑩ 과 달리, 이쪽은 내가 방금 만든 **중복 방어**였으므로
직전 커밋의 「자기 전이 금지 삭제」와 같은 처분을 적용했다. 두 생존을 **다르게 처분한 근거는 「계약
조항인가 내 중복인가」** 다.

**Codex 4R — P1×5 · 4건 완전 수용 · 1건 부분 수용**(3R 이 clean 이 아니라 또 새 축이었다).

| # | 지적 → 확정 | 근거 |
|---|---|---|
| 218 | **post-CAS 결속에 `draftDigest` 대조를 넣는다** | stage·revision·세대·`resultOid` 만 보면 **draft 투영 안의 나머지**(`lifecycle`·`sourceGeneration`·`activeActivity` …)가 롤백·손상돼도 통과하고, 결과 ref 까지 맞으면 `normal-wait` 이 나온다. 저널이 그 전체를 이미 증언하므로(`draftDigest`) **추가 입력 0** 으로 닫힌다 |
| 219 | **저널이 든 `resultRef` **이름 자체**를 검증한다**(문법·bench·txn 결속) — ⚠ **revision 산술은 `composed` 에서만**(⚠ **이 면제는 정정 223 이 폐기했다** — 정답은 면제가 아니라 stage 별 식 `expected + 2 - stageIndex` 였고, 이 행대로 두면 223 이 닫은 구멍(잘못된 `prepared` ref 이름이 그대로 발행)이 되살아난다) | 그 값은 **불변 필드**라, git ref 가 아직 없는 `prepared` 저널의 잘못된 이름이 `no-mutation` 을 타고 살아남아 나중에 **그대로 발행**된다. PR3c 가 문법 소유자가 된 이상 여기서 잡아야 한다. ⚠ **부분 수용**: 지적은 「모든 저널에 `expected + 1`」을 요구했는데 그 등식은 **`composed` 에서만 성립**한다 — ref 는 composed CAS **앞**에서 발행되므로 같은 파일이 `published` 로 전진하면 `expected` 가 올라가 등식이 깨진다. 전 stage 에 걸면 **정상 published 저널이 전부 RED** 다 |
| 220 | **현재 txn 의 ref 이름·OID 대조를 복구표 **앞**으로 끌어올린다** | `published` 저널의 ref 는 revision 이 권위 이하라 앵커 후보가 아닌데, 그 분기가 OID 를 **한 번도 대조하지 않고** `normal-wait` 을 답했다 — 교체·손상된 ref 위에서 시퀀서가 게시를 계속한다. 한 곳에서만 판정하도록 올리고 `classifyComposed` 의 중복은 삭제했다 |
| 221 | **증거 봉인에 저널 레코드 **전체** 다이제스트를 싣는다**(`digestJournalRecord` 신설 · journal.ts) | `draftDigest` 는 **권위 draft 투영**만 덮어서, 저널의 통합 입력(`sourceSnapshot`·`targetBranch`·`targetHeadBeforeIntegration`·`resultTree`)이 관측과 락 사이에 바뀌어도 **같은 증거**가 나온다 — 소비자가 **바뀐 통합 의도**를 같은 것으로 받는다 |
| 222 | **`abandoned` 권위 상태는 종결이다** — 같은 txn 의 되살림 거부 | 파서는 그 값을 여전히 읽는데(레거시·손상) `AUTHORITY_STAGE_ORDER.indexOf('abandoned')` 가 **-1** 이라 `abandoned → prepared` 가 `fi=-1 · ti=0` 으로 역행·건너뛰기 **두 검사를 모두 통과**했다. 소거(청소 복구)는 앞선 분기가 처리하므로 막다른 길이 되지 않는다 |

**이 라운드가 만든 연쇄**: 219 가 서면서 면제 조건 ⑥(관측 ref 의 산술)과 `classifyComposed` 의 산술
검사가 **완전히 가려졌다**(저널 이름이 등식을 강제 + ④ 가 관측 ref 를 그 이름에 묶는다) → 둘 다
**삭제**하고 도달성 증명을 주석으로 남겼다. **뮤테이션 4차 7/7 잡힘** — 생존했던 M33(전-stage ref
**이름** 대조)은 「이름과 OID 를 한 픽스처에서 함께 어긋낸」 탓이라 **축을 분리한 행**을 추가해 닫았다.

**교훈(4R) — 픽스처의 비현실성이 방어의 공백을 가린다.** 이번 라운드에서 깨진 테스트 7건은 전부
**결속상 존재할 수 없는 쌍**을 정상으로 고정하고 있었다(권위는 post-CAS 인데 저널의 draftDigest 는
딴 것 · `composed` 저널의 expected 와 resultRef 가 어긋남). 그런 픽스처 위에서는 새 방어가 **오탐처럼
...
**Codex 6R — P1×6 · 전부 실재 · 전부 수용.** 이번 라운드는 **직전 라운드가 만든 방어의 정의역**에서
전부 나왔다(면제 평가 안에만 있던 검사 · pending 만 보는 benign · abandoned 통째 면제 · `!postCas` 가지).

| # | 지적 → 확정 | 근거 |
|---|---|---|
| 227 | **최초 레코드의 lifecycle 은 `open`** | `prev === undefined` early return 이 완결 귀속 검사들을 **통째로 건너뛴다** — `integrated` + 임의 `completedIntegrationTxnId` 를 든 첫 CAS 가 두 층을 다 통과해 **txn 도 결과도 없이 완결을 주장하는 종결 bench** 를 만들었다. 최초 쓰기에는 대조할 출처가 없으므로 **형태**로 막는다 |
| 228 | **새 txn 시작은 출발지·목적지 **둘 다** `open`** | `prev` 만 보면 **한 CAS 가** T2 를 시작하면서 bench 를 `integrated` 로 넘길 수 있고, 이후 같은-T2 단계 전이는 그 게이트를 **아예 지나지 않는다** |
| 229 | **종결 증거(`abandonedAt`·`abandonReason`)는 admitted 저널 전수에서 본다** | 초안은 그 검사를 **앵커 면제 평가 안에만** 뒀는데, post-CAS `composed` 저널의 ref 는 revision 이 권위와 같아 **후보가 되지 않으므로 그 경로에 영영 도달하지 않는다** → 모순 레코드가 `normal-wait` 을 받았다. ⚠ `publishedAt` 은 `published` 이상에서 **정상**이라 전수 대상이 아니고 면제 평가에 남는다(stage 전제 위에서만 모순) |
| 230 | **pre-CAS 도 결과 상속·draft 결속을 본다** | `!postCas` 가지가 **세대만** 봐서 「결과 A 를 든 composed 권위 + 결과 B 를 든 published 저널」이 순서·산술·ref 대조를 전부 통과했다 — 같은 txn 이 **불변 결과를 갈아치웠다**. 면제 조건 ⑧ 의 draft 재구성을 **전 stage 로 일반화**(`reconstructNextDraft`)해 닫는다 |
| 231 | **`composed` 이후의 포기는 ref 를 요구한다** | `abandoned` 를 통째로 면제하면, 이미 발행된 ref 가 사라진 상태에서 청소 CAS 가 남은 통합 상태를 지우며 **불변 결과의 소실을 숨긴다**. 면제는 「`prepared` 에서의 포기」에만 |
| 232 | **종결 bench 에는 고아 benign 창이 없다** | `integrated`·`archived` 는 current txn 이 없어 `authorityPending` 이 거짓이지만 그 위에서 새 txn 을 **시작할 수 없으므로**(정정 216) 그 저널은 애초에 쓰일 수 없었다 |

**뮤테이션 6차 7/7 잡힘** — 처음엔 M46·M47 이 **생존**했는데, 원인은 픽스처가 **두 축을 동시에** 어긋내
서로 가린 것이었다(결과 B 를 넣으면서 digest 도 함께 어긋났다). 축을 분리한 행을 각각 세워 닫았다.
⚠ 이것은 4R 의 M33 과 **정확히 같은 형태의 재발**이다 — 「한 픽스처에 한 축만」이 규율이 돼야 한다.

**교훈(6R) — 검사의 **위치**가 곧 정의역이다.** 229 는 검사 자체는 옳았는데 **면제 평가 안**에 있어서
그 경로에 닿지 않는 입력을 통과시켰고, 230·231·232 는 전부 「직전 라운드가 세운 가지의 **반대편**」이
비어 있던 경우다. 방어를 추가할 때는 **그 방어가 도달하는 입력 집합**을 함께 적어야 한다.

#### 로컬 적대 리뷰 (2026-08-12 · Codex 리뷰 한도 소진 대체 · 5렌즈 find → refuter verify)

Codex 가 「usage limits for code reviews」로 7R 을 거부해 `fleet-pr-review` 스킬(대체 모드 = 풀 렌즈)로
검증했다. **find 5렌즈 → 후보 약 50건 → refuter 4 + 측정 검증**. 결과: **설계급 P1 1건** · P2 3건 ·
측정 확정 테스트 공백 14건 · P3 다수 · REFUTED 4건.

| # | 확정 | 근거 |
|---|---|---|
| 233 | **설계급 P1 — 결속은 「창」에 따라 강도가 달라야 한다.** 열린 창(`published`·`finalized`)의 post-CAS 는 `revision >= expected + 1` 이고 draft **전체** digest 대조는 **폐기**한다. `finalized` 의 ref revision 은 등식이 아니라 **상한**이다 | 정정 215(3R)·218(4R)·223(5R)가 쌓은 결속이 「통합 단계 사이에 다른 CAS 가 없다」를 전제했는데 `revision` 은 **모든 CAS** 가 +1 하는 전역 카운터다. 스펙은 그 개입을 **요구**한다 — C6(「문자대로 구현하면 모든 integration-ready bench 가 즉시 차단 → D3 자기모순」) · §3-T33(그 창에서 실행 가능) · I8(`archived ∧ integration-ready` 정상). 재현 레코드가 불변식 ④⑤⑥⑦·전이 검사를 **전부 통과**해 커밋 가능하고, 결과는 **정상 상태의 영구 `reconciliation-required`** + 핵심 판정 `resume-composed-cas` 의 **도달 불가**였다. ⚠ 아이러니: `draftBound` 는 4R 에서 「`lifecycle`·`sourceGeneration`·`activeActivity` 롤백을 잡으려고」 넣었는데, 무기한 창에서는 **그 세 필드가 정상적으로 변한다**. ⚠ **닫힌 창은 완화하지 않는다** — 활성 저널이라 실행이 차단되고 복구 판정이 **모든 권위 CAS 보다 먼저** 돈다(정정 197ⓐ). `resultRef` 가 prepare 시점에 revision 을 실을 수 있다는 정정 196 의 전제도 그 사실 위에 선다 |
| 234 | **정리 4건**: ⓐ완결 귀속 검사를 **활성 게이트 밖**으로(종별 `completed-txn-journal-unfinalized` 로 개명) ⓑ면제 ⑨ 의 `publishedAt` 검사 **삭제**(강제자는 `journal.ts` 형태 층 · 측정에서 판정 무변화) ⓒ`abandonedAfterCompose` 의 잉여 disjunct 제거(증거 동결이 둘을 동치로 만들어 **분리 반증 픽스처가 존재하지 않는다**) ⓓblocker 종별 `expected-revision-mismatch` 삭제(생산자 0) | 전부 이 파일이 `arithmetic-binding` 을 지운 잣대(「어휘만 남기면 검사한다로 읽힌다」)의 일관 적용. ⓐ는 6R 정정 229 의 「검사의 위치가 곧 정의역」과 같은 형태였다 |
| 235 | **승격 경로도 현재 txn ref 대조를 받는다** — 대조를 blocker 수집 구간으로 올린다 | 4R 정정 220 이 그 대조를 「표 앞」으로 올렸는데 그 「앞」이 **면제 반환 뒤**였다. 승격 경로에서만 같은 txn 의 여분·손상 ref 가 무검사로 통과했고, 그 상태가 `sealEvidence` 봉인에 실려 **손상이 정당한 증거로 고정**됐다 |

**측정으로 확정된 테스트 반증력 공백 14건 → 전부 해소**(9건 테스트 추가 · 4건 코드 삭제로 소멸 ·
1건 정직 표기). 유형: disjunct/conjunct **가림** 4(종결 증거 2 · 포기 ref 요구 2 · 세대 동치 1) ·
**도달 불가** 1 · 미검증 경로 4(benign lifecycle · finalized 예외 · 정확이름 대조 · 열거성공 가드) ·
봉인 잉여 1 + **미핀** 1 · 진단 미단언 2.

⚠ **남긴 1건(봉인 구분자)의 정직 표기**: 제거해도 테스트가 잡지 못한다 — 봉인에 실리는 값이 전부
고정 길이·고정 문법(ULID 26 · OID 40 hex · sha256 64 hex)이라 **유효 입력으로 경계 충돌을 구성할 수
없다**. 6R 에서 내가 그 논리를 주석에 적고 테스트는 만들지 않은 자리이며, 지금은 「미래에 가변 길이
필드가 추가될 때를 위한 방어이고 현재 독립 반증력은 0」으로 계상한다.

**REFUTED 4건**(근거와 함께 기록 — 재평가 트리거 포함): ⓐ`previousAuthorityStage` 의미 분기로 인한
「탈출 경로 0」 — 그 상태는 정정 204 수렴 규칙을 위반하는 생산자를 요구하고, reconciliation 에서는
`reconcile` 액션이 정의상 존재한다(T80 오독). **트리거**: PR5 가 composed-pending 에서 abandon 을
선행 수렴 없이 착지시키면 즉시 재론 → 그 전에 **정정 204 불변식 ③ 에 `abandon` 을 열거**한다(아래)
ⓑ권위 `abandoned` 미차단 — 소거(청소 복구)는 **설계된 탈출구**이고 blocker 0 경로는 4중 동시 손상을
요구한다 ⓒ중복 `txnId` — 저장 계약(1 txn = 1 파일)상 표현 불가이며 방어 추가는 도달 불가 arm 이다
(**트리거**: PR3d 가 복수 매체를 병합하면 재론) ⓓ앵커 감도 저하 — 「현재 이하 ref = 정상 보존」의 계이며
이미 「확률적 보조 신호 · 안전 근거로 인용하지 않는다」로 정직 표기돼 있다.

**정정 204 불변식 ③ 보강**(REFUTED ⓐ 의 선제 봉쇄): 「current txn 이 composed pending 이면 새
prepare/re-prepare 시작 금지」에 **`abandon` 도 포함**한다 — 포기는 정정 204ⓐ 수렴(동일 composed CAS
재실행) **이후에만** 인가된다. 그러지 않으면 저널의 `previousAuthorityStage`(직전 저널 stage)와 복구가
읽는 권위 stage 가 갈려 판정이 오탐한다.

**교훈(로컬 리뷰) — 「내가 고른 뮤턴트」는 반증력의 증거가 아니다.** 6라운드 동안 뮤턴트 42개로
「41/42 잡힘」을 보고했지만, 새로 추가한 방어의 **각 conjunct·disjunct 를 독립으로** 겨냥하지 않았다.
외부 렌즈가 지정한 14개를 돌리자 **14/14 생존**이었다. 규율: 방어를 추가할 때 **그 방어의 최소 단위마다
한 픽스처씩**(한 픽스처에 한 축만) — 4R M33·6R M46/M47·이번 14건이 전부 같은 형태의 재발이다.
...
**Codex 8R — P1×3 · 전부 실재 · 전부 수용.** 축은 **결속의 형태**다 — 존재 술어(∃)로 세운 검사,
관측을 여러 번 하는 검증, 정의역에 제약을 안 건 arm.

| # | 지적 → 확정 | 근거 |
|---|---|---|
| 242 | **완결 txn 의 보존 ref 도 이름·OID 로 전수(∀) 결속한다** | 정정 239 가 넣은 요구가 **존재 술어**라 ⓐ이름은 맞고 **OID 가 교체**된 ref ⓑ**다른 이름**의 ref ⓒ정상 ref **옆의 여분 손상 ref** 를 전부 통과시켰다 — 완결 txn 은 앵커가 **구조적으로** 답하지 않으므로(완결 CAS 가 revision 을 올려 보존 ref 는 항상 `revision` 이하) 이 층 말고는 그 교체를 볼 곳이 없다. current txn 은 정정 235 의 전수 루프가 같은 손상을 이미 잡고 있었다 — **비대칭이 곧 결함**이었다. ⚠ `completedTxnId === currentTxnId` 면 중복 발화하므로 정의역에서 뺀다. ⚠ **정직 표기**: `integrated → archived` 는 `completedIntegrationTxnId` 를 소거하므로(정상 전이) 보관 이후 보존 ref 는 이 검사의 정의역 밖이다 |
| 243 | **CAS 는 제출 draft 를 한 번만 관측한다** — 사전조건 직후 `{...next}` 스냅숏 | `runCas` 가 `next` 를 사전조건·전이 검증·직렬화에서 **각각 다시 읽었다**. getter·`Proxy` 는 층마다 다른 값을 줄 수 있고 유일한 사후 관문인 왕복 검증은 `checkInvariants`(**단일 레코드**)만 태우므로, **전이 계층이 `prepared` 를 검증한 CAS 가 `published` 를 착지시켰다**(실측 재현). 전이 계층 전체가 「제출된 값을 본다」는 전제 위에 서 있으므로 그 전제를 코드로 세운다 |
| 244 | **미종결 통합의 소거는 lifecycle 을 전진시킬 수 없다** | 소거 arm 이 목적지 lifecycle 에 아무 제약도 안 걸어, 한 CAS 가 미종결 통합을 지우면서 동시에 `archived` 로 넘어갈 수 있었다 — 그러면 보존된 그 txn 의 저널이 **종결 bench 위의 활성 고아**가 되어 복구가 즉시 reconciliation 을 답한다. 6R 정정 232 가 복구 쪽에서 잡던 상태를 **전이 검증기 자신이 제조**한다. 정정 228(「새 txn 은 open bench 에서만」)의 **대칭**이다. ⚠ 정의역은 미종결에만 — `finalized` txn 의 소거는 완결·보관 경로의 정상 동반 변경이다 |
| 245 | **거짓 근거 주석 정정**(동작 변화 0) | 6R 정정 232 의 주석이 「종결 bench 위의 그 저널은 **애초에 쓰일 수 없었다**」고 적었는데 거짓이다 — 그 저널은 bench 가 `open` 이던 시절에 합법으로 쓰였을 수 있고, 전이 층이 막는 것은 **새 txn 시작**뿐이다(그리고 244 이전에는 종결화 자체도 안 막았다). 참인 근거는 「종결 bench 위에서는 재개·재준비가 인가되지 않으므로 처분을 사람에게 넘긴다」. **검증 안 된 안전 주장의 문서화**를 지우는 정정 133·152·156 계열 |

**교훈(8R) — 「검사가 있다」와 「검사가 전수다」는 다르다.** 242 는 내가 7R 에서 **존재 술어**로 세운
검사였고, 같은 파일 안에 이미 **전수 루프**(정정 235)가 있었는데 그 형태를 복제하지 않았다. 규율:
**같은 손상 축에 이미 판정이 있으면 그 형태를 그대로 일반화**한다 — 새 술어를 쓰는 순간 정의역이 갈린다.
...
- **T12 R-5 = 신규 연산 한정 재시도**(정정 114·115 가 원문을 대체) — §3-T58 은 **ref `.lock` 경합 +
  ...
  삭제 분기가 도달 불가다.
> ⚠ **아래 T13 원문은 정정 132~148 이 대체했다.** 원문은 ⓐ정정 123 의 미결 설계 결정을 안고 있었고
> ⓑ이월 5건(70·78ⓒ·85·86ⓑ·PR3a 스테일 ref 락)을 **0건** 반영했으며 ⓒ네 계약 테스트가 PR3b 에서
> vacuous 임을 몰랐다. 실제 착수 지시서는 아래 **T13a·T13b·T13c** 다.

- ~~**T13 통합 WAL 저널 + 복구 판정 + D/F 프로브**~~ — 원문 보존: §3-T32(6조합 + `REF_NAMESPACE_CONFLICT`
  **최우선**) · §3-T23 · §3-T33 · §3-T34 · §3-T63 · 생성 저널 3채널 열거 → `never-applied` 종결.

> ⚠ **T13a 는 정정 159 가 축소했다** — 아래 ⓔ(복구 판정)·ⓕ 의 판정 관련분은 **PR3c(T13b′)로 이동**했다.
...
- **T13a 저널 코어 + ~~복구 판정~~ (PR3b)** — 신설 `src/main/core/workbench/journal.ts`(정정 141).
  ...
  **활성 저널 집합 = {`prepared`,`composed`} 순수 술어**(§3-T33 의 PR3b 구성 가능분) ⓔ**복구 판정 순수
  함수** — 정정 136 의 **5행 전수** + `REF_NAMESPACE_CONFLICT` 최우선 + 정정 134 의 **매체 불일치 2행**.
  §3-T63 은 정정 134 대로 **2행 분해**하되 「저널↔ref」 분은 **PR5 T18 귀속**(정정 137).
  **정직 표기 의무**: 이 PR 의 vacuous 목록(§3-T63 ref 분 · T18b 후반 연언 · T33 네 소비자 · T20 raw 분 ·
  T29)을 **PR 본문에 사전 계상**한다.
  ⓕ**2차 라운드 반영(정정 150·154·155ⓓ·157)**: 복구 판정 시그니처는 **`{record, journalEntries, refs} →
  verdict`** 이고 앵커 입력이 **처음부터 필수**다(미주입 = `anchor-unavailable` **fail-closed** · 옵셔널
  금지). `refs` 는 **두 열거를 모두** 싣는다 — 접두 `…/<benchId>/` 와 **정확 이름** `…/<benchId>`(실측:
  접두 질의는 bare 부모를 **0건**으로 답한다). **abort-on-CAS-failure** 를 명시 계약으로 둔다(각 단계 =
  (저널 쓰기, 권위 CAS) 쌍 · 어느 쪽이 실패해도 **다음 단계로 전진하지 않는다**). **귀속 규칙**: 저널
  엔트리는 권위가 참조할 때만 활성이고, 귀속 없는 `prepared`/`composed` = **`never-applied`** 회수 적격.
  매체 비교 정의역 = **current txn 1건**. 그리고 **저널 tmp 문법 `<txnId>.json.<ownerToken>.tmp` +
  「저널 쓰기는 bench 리스 아래에서만」을 이 PR 의 계약으로 못박는다**(PR3d 의 선행조건).
> ⚠ **아래 T13b 원문은 정정 190~207 이 대체했다**(폐기 어휘 재유입 금지 — 읽는 순서를 여기서 못박는다):
> 앵커 정렬 키는 `highestIntegrationTxnId`(ULID)가 **아니라 권위 `revision`** 이고(정정 190), 따라서
> **레코드 필드 신설도 · `prepared` 시점 선갱신도 · strict 전진 강제도 없다**. 「발화 조건 =
> `durability === 'file-only'`」도 **폐기**됐다(정정 205 — 판정은 항상 돈다). 실제 착수 지시서는
> **정정 190~204 + 착지 실측 205~207** 이다.

- ~~**T13b ref-앵커 (PR3c)**~~ — 원문 보존: `BenchAuthorityRecord.highestIntegrationTxnId` 신설 · 갱신 시점 **`prepared`** ·
  **연산 = strict 전진 강제**(정정 149 가 `max` 를 폐기했다 — `T_new > old.highest` 를 **전이 불변식**으로
  요구하고 위반 시 fail-closed) + **ULID 워터마크 클램프 seam**(`now = max(Date.now(), decodeUlidTime(highest)+1)`
  — 시계 역행을 흡수) · **전이 불변식 계층 신설**(신·구 레코드 비교 — 현행 `checkInvariants` 는 단일 레코드
  전용) · 발화 조건은 **`durability === 'file-only'`** 이고 **`platform === 'win32'` 가 아니다**(정정 151) ·
  §3-T61 전수표·exact 키 핀 개정. **같은 커밋에서 §3-T34·T18b 문면을 개정**한다(정정 153 — 안 하면 RED
  목록이 자기모순이고 구현자가 판정식을 구 형태로 되돌린다). **authority.ts:107-108 의 거짓 주석은
  PR3b 첫 커밋에서 이미 정정**되어 있어야 한다(정정 133·152·156 — 근거는 git **소스** 인용으로).
- **T13c 크래시 tmp 수확기 + 스테일 ref 락 관측 (PR3d)** — `DurableFs` **11번째 프리미티브**(열거) 신설 ·
  §W-5 개정 · **2표면만**(권위 tmp · 저널 tmp) · 수확은 **레포 1패스가 아니라 per-bench** — 「그 benchId 의
  리스를 보유한 구간에서 그 benchId 것만 · `ownerToken` 불일치」(정정 155 · 나이·mtime 휴리스틱 금지) ·
  **영역·인스턴스 tmp 는 수확 비대상**(이름에 소유 증거가 없어 안전한 판정식이 부재 · 생성이 레포당 1회라
  누적 유계 — **비목표로 명시 선언**) · 스테일 ref 락은 **관측·보고만**(삭제 금지 — R-5·§W-7:778) ·
  §3 행 **신설**(현재 0건 — §1 전제 1 게이트). **선행조건**: PR3b 가 저널 tmp 문법을 확정해야 착수 가능.
- **생성 저널 3채널**(정정 143 → **정정 182 가 귀속을 바꿨다**) — ①저널 엔트리 ②worktree 디렉터리
  ③git 브랜치. **판정 규칙과 행동 단언을 함께 PR4 T16 에 착지**시킨다(§3-T72). 규칙만 PR3b 로 먼저 빼면
  「판정이 자기 입력의 생산자보다 먼저 서는」 배치가 되어 정정 159 가 제거한 안티패턴이 되살아나고,
  대상 레코드도 통합 WAL 이 아니라 **생성 저널**이라 PR3b 모듈의 계약이 아니다.
