...
⚠ **차기 PR 에 상속 금지**를 재확인한다 — PR2c~PR7 은 이 초과를 추정 근거로 쓰지 않는다. 그리고 이 PR 이
남긴 교훈을 §1-6 에 반영한다: **「착수 전 예측은 리뷰 하드닝을 포함하지 않는다」** — 실측 팽창률은
2,020 → 2,735 = **1.35배**이므로, 사전 추정에는 그 계수를 곱해 상한을 확인해야 한다(PR1b 는 1,581→2,430 =
1.54배였다 · 밴드 **1.35~1.54**).
