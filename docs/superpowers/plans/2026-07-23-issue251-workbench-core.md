...
**판정**: 프록시는 「직접 측정이 불가능한 시점」에만 권위다(그것이 프록시의 정의다). 착지 후에는 §1-6 이
지정한 직접 측정이 **상위 권위**이고 그 값은 상한 이내다 → **분할점을 발동하지 않는다**. 다만 두 가지를
은폐하지 않는다: ⓐ여유가 **13행**뿐이고 ⓑ정정 74 가 기록했듯 **리뷰 라운드가 코드를 늘린다**(PR1b 는
1,581→≈2,430). 따라서 이 PR 의 리뷰 반영은 **행 증가를 명시 예산으로 관리**하며, 초과가 확정되면 그때
정정 92 의 2차 레버(읽기 경로/쓰기 경로 분리)를 적용한다. 밴드 자체는 이 관측(2.15)을 포함해
**2.15~3.34** 로 갱신하되, **하한을 예측에 쓰지 않는다**(하한은 사후 설명이지 사전 보증이 아니다).
