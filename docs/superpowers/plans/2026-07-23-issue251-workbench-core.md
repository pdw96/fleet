...
| **PR3a** | `GitRepo` 완성(열거 포함 10메서드) · 능력 프로브 · R-5 삭제 없는 재시도 | T11·T12 | **실측 착지 768**(프로덕션 248) | 없음(상한 내) |
...
| 119 | 계획 :940 「나머지 **8**메서드」 | 스펙 인터페이스 전수 계수는 **11**(착지 2 제외 시 9)이고, 정정 113 의 `listRefs` 를 더하면 **10** 이다 | 「나머지 **10**메서드」로 정정. 완료 조건을 계수로 두면 누락이 PR5 착수 시점에야 발각된다 |
...
- **분량**: 순증 **768**(프로덕션 물리행 **248** = `git.ts` 243 + `coord-area.ts` 5 · 테스트 520).
  상한 1,900 의 **40%** · 착지 목표(60~65%) **이내**. 안전선 569 대비 프로덕션 248 로 여유가 크다 —
  BUD 렌즈 추정(525)이 **2.1배 과대**였다는 실측이 남는다(다음 PR 추정의 보정 계수).
- **게이트**: verify 7게이트 GREEN · vitest **2575 pass / 57 skip**(108 파일) ·
  커버리지 **S93.46 B87.17 F94.64 L94.90**(floor 91/83/90/92 · 착수 기준선 S93.48 B87.22 F94.53 L94.88
  대비 S−0.02 B−0.05 **F+0.11 L+0.02** — 사실상 평행. 신규 모듈이 자체 커버를 충족했다는 뜻).
- **뮤테이션 9종 전부 RED**(위 정정 126 의 2종 보강 후). 값 큰 것: 「merge-tree 판별을 종료코드로」·
  「refExists 를 rev-parse 로」·「is-ancestor 128 을 no 로」·「재시도 제거」·「재시도 범위를 모든 실패로」·
  「열거 exact 를 접두로」·「능력 프로브 항상 true」·「왕복 검증 제거」.
...
- **T11 `GitRepo` 나머지 10메서드 + 능력 프로브**(정정 119 — 원문 「8」은 계수 오류이고 정정 113 이
