...
**교훈 — `npm run verify` 는 소스의 NUL 바이트를 보지 않는다.** `recovery.ts` 의 템플릿 리터럴 안에
U+0000 이 섞인 채로 **prettier · tsc · eslint · vitest · build 가 전부 통과**했고, 발견 경로는 게이트가
아니라 `grep` 이 그 파일을 「binary file」로 답한 것이었다. 게이트 전수가 무신호인 오염 축이 하나 열려
있다는 뜻이다(영향: 진단 도구 오작동 · diff 가독성). **#251 밖 후속 후보로 등재**한다 — 이 PR 에서
고치면 축이 하나 늘어난다.
