...
- **D2 — 표시 = 절대경로 항상 표시 + 비절대경로만 경고 플래그.** "shadow 탐지"가 아니라 **"resolved path 표시 + 상대 PATH 위험 플래그"**로 정직하게 명명(Codex 보강). UI 문구 3단(§7).
...
- **D6 — 방어적 해석 + 동시 실행.** `which` not-found → `{nothrow:true}` null(throw 안 함), 예외 → try/catch 삼킴. **`which` 실패가 `--version` 감지를 깨지 않는다**(resolved-path는 부가 정보). 버전 spawn과 `Promise.all` 동시 실행(추가 지연 0). `installed:true & which null`(드문 해석 불일치, 예: Windows App Paths) → `resolvedPath` undefined·`pathShadowRisk` undefined → UI "확인할 수 없음".
...
const defaultResolver: PathResolver = (command) => which(command, { nothrow: true })
```
...
2. **`pathShadowRisk === true` 일 때만 강한 경고**(`role="alert"`): `상대 PATH 항목에서 해석되었습니다. 현재 작업 디렉터리에 따라 다른 실행 파일이 선택될 수 있습니다.`
...
- which not-found → null → 필드 undefined.
- which 예외 → 삼킴, 탐지 정상.
- `installed:true & which null` → 경로 "확인할 수 없음", 크래시 없음.
...
- `pathShadowRisk` 는 **상대 PATH 해석만** 경고한다. **절대경로에 심긴 shadow 는 자동 판정하지 않는다**(사용자 육안 검증 — 보조문구로 고지).
