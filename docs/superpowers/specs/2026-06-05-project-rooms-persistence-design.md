...
## 2.5 실행 지속성 (Run continuity) — 코드로 확인됨

핵심 요구사항: **"다른 탭으로 이동해도 프로젝트 작업(실행)은 계속 돌아가야 한다."**

**이미 충족된다 — 실행이 렌더러가 아니라 메인 프로세스에 있기 때문.** `runProjectFlow → runProject`는 메인 프로세스(Electron Node)에서 실행되며(`engine.ts:240`), 렌더러 컴포넌트 수명과 분리돼 있다:
- 탭 전환 → `ProjectPanel` 언마운트(렌더러)되지만 메인의 오케스트레이션 async는 **계속 실행**하며 store에 기록 + 이벤트 broadcast.
- 실행이 멈추는 유일한 경로는 **명시적 `cancelRun`**(취소 버튼, `engine.ts:295`의 AbortController.abort). 탭 전환은 이를 건드리지 않는다.

따라서 본 작업의 역할은 *실행을 지속시키는 것*이 아니라(이미 지속됨) **돌아왔을 때 "계속 돌고 있었다"가 보이도록 view를 재부착**하는 것이다(§3.4 마운트 재조회 + projectId 라이브 재부착).

**경계(사용자 확인됨, 2026-06-05): 탭 전환만 보장. 앱 완전 종료/재시작 시 run은 중단되며 재개하지 않는다** — 메인 프로세스가 죽으면 in-memory run 상태(AbortController·activeRuns)가 사라진다. 앱 재시작 후 이어실행은 세션·워크스페이스 영속화를 동반하는 별도 단계로, 본 스펙 범위 밖.

> 부가 메모: 백그라운드 run이 destructive **승인 게이트**(`gate.request`)에 걸리면 사용자가 응답할 때까지 그 작업은 대기한다(승인 모달은 앱 레벨 broadcast이므로 다른 탭에서도 받음). 본 작업이 바꾸지 않는 기존 동작.
