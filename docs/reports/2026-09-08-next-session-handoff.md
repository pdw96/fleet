...
- **#290** — 제어문자 가드 확장(NUL → C0/C1). 2026-08-12 이후 정지. **재개 가능함을 실측했다**:
  ...
  불필요하다.
  재트리거한 Codex 가 **P1 을 두 건** 냈고 둘 다 수용해 그 브랜치에 얹었다(head `952b057` →
  `39e91b4` → `1ab3f9c`, 사용자 승인 후 푸시):
  ① 스캔 `ROOTS` 가 `.claude` 를 빼고 있었다 — 그 트리는 eslint 가 통째로 `ignores` 하고
  `skills:lint` 에도 제어문자 검사가 없어 **어떤 게이트도 보지 않는데**, 그 안에 머지 게이트
  본체(`.claude/hooks/require-codex-review.mjs`)가 있다. 가드가 가장 위험한 파일만 비켜 갔다.
  ② CR 허용의 근거(「win32 CRLF 체크아웃 때문에 필수」)가 **실물과 어긋났다** — `.gitattributes`
  의 `* text=auto eol=lf` 가 워킹트리를 전 플랫폼 LF 로 정규화하고 그 파일은 base 에도 이미
  있었다. 반면 단독 CR 은 JS 에서 LineTerminator 라 `//` 주석을 조용히 끝낸다. CR 을 거부로
  돌렸고, 유일한 미검증 축이던 windows 레그도 통과해 전제가 실증됐다.
  현재 `1ab3f9c` 에서 체크 6개 green. 남은 건 그 head 에 결속된 Codex 재리뷰뿐이다.
