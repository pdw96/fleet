...
## 결정
솔로 + pre-1.0 단계에선 투기적이거나 net-new 가치가 입증되지 않은 단순 패리티 누수는 등재하지 않는다.
`register-now` 급은 가치가 입증된 net-new 발굴에서만 나온다(11차 #121 처럼 비대칭 보호 누수라도 실가치가
입증되면 등재됨). 과설계 경계를 명시적 ROI 게이트로 강제.
